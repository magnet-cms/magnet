 ...
 */
export const SchemaBuilderContext = createContext<SchemaBuilderContextType | null>(null)
...
export function useSchemaBuilderState(
  ...
  const addField = useCallback((partial: Partial<SchemaField>) => {
    ...
    const field: SchemaField = {
      ...
      displayName,
      type: fieldType,
      tsType: definition?.tsType || FIELD_TYPE_TO_TS_TYPE[fieldType] || 'string',
      prop: {
        required: false,
        unique: false,
        ...partial.prop,
  ...
  const updateField = useCallback((id: string, changes: Partial<SchemaField>) => {

 ...
 */
function schemaMetadataToBuilderState(name: string, metadata: any): SchemaBuilderState {
  return {
    schema: {
        ...
        actions={
          isReadOnly ? undefined : (
            <>
              ...
              <Button size="sm" onClick={handleDeploy} disabled={!state.schema.name}>
      ...
      {isReadOnly && (
        <div className="px-6 py-2 border-b border-border bg-amber-50 dark:bg-amber-950/30 flex items-center gap-2">
          ...
          <span className="text-xs text-amber-700 dark:text-amber-400">
            This schema is defined in code and cannot be modified here. View the generated code in
            the Code tab to copy and customize.
          </span>
        </div>
      )}
          ...
          {state.viewMode === 'builder' ? (
            <PageContent className="p-6 overflow-y-auto flex-1">
              <div className="space-y-4">
                ...
                {!isReadOnly && (
                  <div className="p-3 bg-blue-50 border border-blue-100 rounded-lg flex items-start gap-3">
                    ...
                    <div className="text-xs text-blue-900">
                      <span className="font-medium">Schema Guidelines:</span> Keep field names in
                      camelCase for better compatibility.
                    </div>
                  </div>
                )}
                ...
                {/* Field List */}
                <FieldList
                  onAddField={isReadOnly ? undefined : () => setAddFieldOpen(true)}
                  readOnly={isReadOnly}
                />
              </div>
            </PageContent>

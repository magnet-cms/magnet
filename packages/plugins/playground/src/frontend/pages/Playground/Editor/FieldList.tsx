...
import { Sortable, SortableContent, SortableItem, SortableOverlay } from '@magnet-cms/ui/components'
            ...
            {state.fields.map((field) => (
              <SortableItem key={field.id} value={field.id}>
                <FieldCard
                  ...
                  onDelete={() => {
                    if (window.confirm(`Are you sure you want to delete "${field.displayName}"?`)) {
            ...
            {({ value }: { value: UniqueIdentifier }) => {
              ...
              return field ? (
                <div className="bg-background border rounded-lg shadow-lg">
                  <FieldCard field={field} isSelected showActions={false} showDragHandle={false} />
                </div>
              ) : null
            }}
          </SortableOverlay>

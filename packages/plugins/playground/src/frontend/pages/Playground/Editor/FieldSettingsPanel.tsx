...
import type { FieldType, RelationConfig, ValidationRule } from '../types/builder.types'
...
export function FieldSettingsPanel() {
  ...
  const handleAddOption = () => {
    ...
    updateField(selectedField.id, {
      ui: {
        ...selectedField.ui,
        options: [...currentOptions, { key: newOptionKey.trim(), value: newOptionValue.trim() }],
      },
    })
    setNewOptionKey('')
    setNewOptionValue('')
  ...
  const handleUpdateValidationConstraints = (type: string, constraints: (string | number)[]) => {
    updateField(selectedField.id, {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="p-4 border-b flex items-center justify-between">
        <h2 className="text-xs font-semibold uppercase tracking-wide">Field Settings</h2>
        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => selectField(null)}>
          <X className="h-4 w-4" />
        </Button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <Accordion type="multiple" defaultValue={['basic-info', 'constraints']} className="w-full">
          {/* Basic Info */}
              ...
              {uiSubtypes.length > 1 && (
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {uiSubtypes.find((s) => s.id === selectedField.ui.type)?.description ||
                      uiSubtypes[0]?.description}
                  </p>
                </div>
              )}
            </AccordionContent>
            ...
            <AccordionItem value="relation-config" className="px-5">
              ...
              <AccordionContent className="space-y-4">
                ...
                <div className="p-3 bg-muted/50 rounded-lg border space-y-3">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-medium">
                      {getRelationTypeLabel(selectedField.relationConfig?.relationType)}
                    </span>
                  </div>
                    ...
                    onValueChange={(value) =>
                      updateField(selectedField.id, {
                        relationConfig: {
                          ...
                          targetSchema: value,
                          relationType: selectedField.relationConfig?.relationType || 'manyToOne',
                        },
                      })
                    }
                  >
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={selectedField.relationConfig?.relationType || 'manyToOne'}
                    onValueChange={(value) =>
                      updateField(selectedField.id, {
                        relationConfig: {
                          ...selectedField.relationConfig,
                          targetSchema: selectedField.relationConfig?.targetSchema || '',
                  ...
                  >
                    ...
                    {ruleDef?.constraintCount && ruleDef.constraintCount > 0 && (
                      <div className="flex gap-2">
                        {Array.from({ length: ruleDef.constraintCount }).map((_, i) => (
                          <Input
                            ...
                            placeholder={ruleDef.constraintLabels?.[i] || `Arg ${i + 1}`}
                            value={validation.constraints?.[i] ?? ''}
                            onChange={(e) => {
                              const newConstraints = [...(validation.constraints || [])]
                              newConstraints[i] =
                                validation.type.includes('Length') ||
                                validation.type.includes('Min') ||
                                validation.type.includes('Max')
                                  ...
                                  : e.target.value
                              handleUpdateValidationConstraints(validation.type, newConstraints)
                            }}
                            className="flex-1"
                          />
                        ))}
              ...
              <Select onValueChange={handleAddValidation}>
                ...
                <SelectContent>
                  {availableValidations
                    .filter((v) => !selectedField.validations.some((sv) => sv.type === v))
                    .map((validation) => {

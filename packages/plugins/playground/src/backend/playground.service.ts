...
/**
 * Options for `@magnet-cms/plugin-playground` (`PlaygroundPlugin.forRoot()`).
 */
...
export class PlaygroundService {
  ...
  constructor(
    ...
    private readonly magnetOptions?: MagnetModuleOptions,
    @Optional()
    @InjectPluginOptions('playground')
    private readonly pluginOptions?: PlaygroundPluginOptions,
  ) {}
  ...
  private getModulesDir(): string {
    ...
    // Fall back to magnet module options
    const opts = this.magnetOptions as {
   ...
   */
  detectConflicts(existing: SchemaFieldDto[], updated: SchemaFieldDto[]): ConflictInfo[] {
...
`
  ...
  private generateDtoProperty(field: SchemaFieldDto): string {
    ...
    for (const v of field.validations) {
      if (['IsString', 'IsNumber', 'IsBoolean', 'IsDate', 'IsNotEmpty'].includes(v.type)) {
   ...
   */
  private generateSchemaDecorator(options?: CreateSchemaDto['options']): string {
    const opts: string[] = []
  ...
  private generateUIOptions(field: SchemaFieldDto): string {
    ...
    if (field.ui.options && field.ui.options.length > 0) {
      const optionsStr = field.ui.options
        .map(
          (o) => `{ key: '${this.escapeString(o.key)}', value: '${this.escapeString(o.value)}' }`,
        )
        .join(', ')
   ...
   */
  private inferFieldType(tsType: string, uiStr: string): SchemaFieldDto['type'] {

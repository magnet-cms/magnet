...
export class SentryApiService {
	...
	async getOrgStats(): Promise<SentryProjectStats> {
		if (!this.isOrgConfigured()) {
			throw new Error(
				'Sentry API not configured: authToken and organization are required',
			)
		}
	...
	async getOrganizationProjects(): Promise<SentryProject[]> {
		...
		return raw.map((p) => ({
			...
			errorCount: p.stats
				? p.stats.reduce((sum, [, count]) => sum + count, 0)
				: null,
		}))
	}
	...
	async probeTokenScopes(): Promise<SentryTokenScopes> {
		...
		const [orgRead, projectRead, alertsRead] = await Promise.all([
			...
			probe(
				`${this.baseUrl}/api/0/organizations/${this.organization}/projects/?per_page=1`,
			),
			probe(
				`${this.baseUrl}/api/0/organizations/${this.organization}/alert-rules/?per_page=1`,
			),
		])

...
const SubscriptionsPage = () => {
  ...
  useEffect(() => {
    async function fetch() {
      try {
        const data = await adapter.request<Subscription[]>('/polar/admin/subscriptions')
  ...
  const handleCancel = async (polarSubscriptionId: string) => {
    try {
      await adapter.request(`/polar/admin/subscriptions/${polarSubscriptionId}/cancel`, {
        method: 'POST',
      })
      setSubscriptions((prev) =>
        prev.map((s) =>
          s.polarSubscriptionId === polarSubscriptionId ? { ...s, status: 'revoked' } : s,
        ),
      )
    } catch (error) {

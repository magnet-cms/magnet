...
export class PolarSubscriptionService {
  ...
  async syncSubscription(data: {
    ...
    if (existing) {
      return this.subscriptionModel.update(
        { polarSubscriptionId: data.id } as Partial<BaseSchema<PolarSubscription>>,
        subData as Partial<BaseSchema<PolarSubscription>>,
      )
    }

    return this.subscriptionModel.create(subData as Partial<BaseSchema<PolarSubscription>>)
  }
   ...
   */
  async findActiveByCustomerId(customerId: string): Promise<BaseSchema<PolarSubscription> | null> {

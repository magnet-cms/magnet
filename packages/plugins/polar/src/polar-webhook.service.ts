    ...
    switch (event.type) {
      ...
      case 'customer.updated':
        await this.customerService.upsertFromWebhook({
          ...
          metadata: event.data.metadata as Record<string, string | number | boolean>,
        })
        break
      ...
      case 'benefit_grant.updated':
        await this.benefitService.syncBenefitGrant({
          ...
          grantedAt: event.data.isGranted ? (event.data.modifiedAt ?? new Date()) : undefined,
        })
        break
      ...
      default:
        this.logger.log(`Unhandled event type: ${(event as { type: string }).type}`)
    }
  }

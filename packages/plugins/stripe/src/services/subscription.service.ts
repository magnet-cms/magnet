...
export class StripeSubscriptionService {
   ...
   */
  async syncSubscription(stripeSub: Stripe.Subscription): Promise<BaseSchema<StripeSubscription>> {
    const customerId =
    ...
    if (existing) {
      return this.subscriptionModel.update(
        { stripeSubscriptionId: stripeSub.id } as Partial<BaseSchema<StripeSubscription>>,
        data as Partial<BaseSchema<StripeSubscription>>,
      )
    }

    return this.subscriptionModel.create(data as Partial<BaseSchema<StripeSubscription>>)
  }
   ...
   */
  async findActiveByCustomerId(customerId: string): Promise<BaseSchema<StripeSubscription> | null> {

...
const PaymentsPage = () => {
  ...
  const handleRefund = async (paymentIntentId: string) => {
    try {
      await adapter.request(`/stripe/admin/payments/${paymentIntentId}/refund`, { method: 'POST' })
      setPayments((prev) =>
        prev.map((p) =>
          p.stripePaymentIntentId === paymentIntentId ? { ...p, status: 'refunded' } : p,
        ),
      )
    } catch (error) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-background h-full relative overflow-hidden">
      <PageHeader title="Payments" description={`${payments.length} payment(s) total.`} />

...
const SubscriptionsPage = () => {
  ...
  useEffect(() => {
    async function fetch() {
      try {
        const data = await adapter.request<Subscription[]>('/stripe/admin/subscriptions')
  ...
  const handleCancel = async (stripeSubscriptionId: string) => {
    try {
      await adapter.request(`/stripe/admin/subscriptions/${stripeSubscriptionId}/cancel`, {
        method: 'POST',
      })
      setSubscriptions((prev) =>
        prev.map((s) =>
          s.stripeSubscriptionId === stripeSubscriptionId ? { ...s, status: 'canceled' } : s,
        ),
      )
    } catch (error) {

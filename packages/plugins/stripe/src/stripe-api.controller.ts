...
export class StripeApiController {
   ...
   */
  @Post('checkout')
  async createCheckout(@Body() dto: CreateCheckoutDto): Promise<SessionResponse> {
    try {
  ...
  @Get('access/:userId')
  @RestrictedRoute()
  async getAccess(@Param('userId') userId: string): Promise<SubscriptionAccessResponse> {
    ...
    await this.paymentModel.update(
      {
        ...
        import('@magnet-cms/common').BaseSchema<import('./schemas/payment.schema').StripePayment>
      >,
      {
        status: 'refunded',
      } as Partial<

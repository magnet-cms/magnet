...
export class StripeWebhookService {
  ...
  private async dispatchEvent(event: Stripe.Event): Promise<void> {
    switch (event.type) {
      ...
      case 'customer.updated':
        await this.customerService.upsertFromStripe(event.data.object as Stripe.Customer)
        break

      case 'customer.deleted':
        await this.customerService.deleteByStripeId((event.data.object as Stripe.Customer).id)
        break

      case 'customer.subscription.created':
      case 'customer.subscription.updated':
        await this.subscriptionService.syncSubscription(event.data.object as Stripe.Subscription)
        break
      ...
      case 'product.updated':
        await this.productService.syncProduct(event.data.object as Stripe.Product)
        break

      case 'product.deleted':
        await this.productService.deleteProduct((event.data.object as Stripe.Product).id)
        break
      ...
      case 'price.deleted':
        await this.productService.deletePrice((event.data.object as Stripe.Price).id)
        break
  ...
  private async recordProcessedEvent(eventId: string, eventType: string): Promise<void> {
    try {

...
export class SupabaseStorageAdapter extends StorageAdapter {
  ...
  constructor(config: SupabaseStorageConfig) {
    ...
    if (!config.supabaseUrl || !config.supabaseKey || !config.bucket) {
      throw new Error('Supabase storage requires supabaseUrl, supabaseKey, and bucket in config')
    }
  ...
  async initialize(): Promise<void> {
    ...
    if (!bucketExists) {
      const { error } = await this.supabase.storage.createBucket(this.config.bucket, {
        public: true,
      })
      if (error) {
  ...
  async deleteMany(paths: string[]): Promise<{ deleted: number; failed: string[] }> {
    const { error } = await this.supabase.storage.from(this.config.bucket).remove(paths)
  ...
  async getStream(path: string): Promise<Readable> {
    const { data, error } = await this.supabase.storage.from(this.config.bucket).download(path)
  ...
  async move(path: string, newPath: string): Promise<UploadResult> {
    const { error } = await this.supabase.storage.from(this.config.bucket).move(path, newPath)
  ...
  async copy(path: string, newPath: string): Promise<UploadResult> {
    const { error } = await this.supabase.storage.from(this.config.bucket).copy(path, newPath)
   ...
   */
  static forRoot(config?: Partial<SupabaseStorageConfig>): StorageMagnetProvider {

...
import type { Document, FilterQuery as MongooseFilterQuery, Model as MongooseModel } from 'mongoose'
 ...
 */
function isCastError(error: unknown): error is { name: 'CastError'; path: string; value: unknown } {
  return (
    isObject(error) &&
    'name' in error &&
...
export class MongooseQueryBuilder<T> extends QueryBuilder<T> {
  ...
  private async resolveModel(): Promise<MongooseModel<Document & BaseSchema<T>>> {
    return this.modelOrPromise instanceof Promise ? this.modelOrPromise : this.modelOrPromise
  }

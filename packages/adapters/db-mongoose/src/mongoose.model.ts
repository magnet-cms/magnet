...
export function createModel<T>(
  ...
  return class MongooseModelAdapter extends Model<T> {
    ...
    async _ensureModel(): Promise<MongooseModel<Document & BaseSchema<T>>> {
      ...
      const promise: Promise<MongooseModel<Document & BaseSchema<T>>> = (async () => {
        // If it's already a model instance, use it
        if (typeof this._modelInstance !== 'function') {
          this._model = this._modelInstance as MongooseModel<Document & BaseSchema<T>>
          return this._model
        }
    ...
    get _modelSync(): MongooseModel<Document & BaseSchema<T>> {
      if (!this._model) {
        throw new Error('Model not initialized. Ensure model is ready before use.')
      }
      return this._model
    }
    ...
    async update(
      ...
      const result = await model.findOneAndUpdate(mongooseQuery, mongooseData, { new: true }).lean()
    ...
    async delete(query: Partial<BaseSchema<T>>): Promise<boolean> {
      ...
      if (result && this.isVersioningEnabled()) {
        ...
        if (resultId) {
          await this.createVersion(resultId, mapDocumentId(result.toObject()), 'archived')
        }
      }
    ...
    native(): NativeAccess<T> {
      ...
      return {
        ...
        async rawQuery<R = unknown>(query: string, _params?: unknown[]): Promise<R> {
          ...
          const result = await (model.aggregate as (pipeline: unknown[]) => Promise<unknown[]>)(
            pipeline,
          )
          return result as R
        },
    ...
    getMetadata(): SchemaMetadata {
      ...
      const schemaOptions = (this._model.schema as unknown as { options?: Record<string, unknown> })
        .options
      const schemaClass = schemaOptions?.schemaClass as (new () => unknown) | undefined

...
export class InternationalizationService {
  ...
  private transformPath(
    ...
    schema
      .virtual(path)
      .get(function () {
        const locale = this.__locale || schema.methods.getDefaultLocale() || defaultLocale
   ...
   */
  private addSchemaMethods(schema: Schema, locales: string[], defaultLocale: string): void {
    ...
    schema.static('getDefaultLocale', () => schema.statics.__defaultLocale || defaultLocale)
  }
  ...
  private addSchemaHooks(
    ...
    schema.pre('save', function (next) {
      const requiredPaths = Object.keys(intlPaths).filter((path) => intlPaths[path].required)
      const currentDefaultLocale = schema.methods.getDefaultLocale() || defaultLocale
      ...
      for (const path of requiredPaths) {
        ...
        if (
          ...
          return next(
            new Error(`Path '${path}' is required in default locale '${currentDefaultLocale}'`),
          )
        }
      }
      next()
  ...
  getSchemaDefaultLocale(schema: Schema): string {
    return this.schemaDefaultLocales.get(schema) || this.defaultOptions.defaultLocale
  }
}

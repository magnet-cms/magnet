...
export function applyIntl(schema: Schema, options: Partial<IntlOptions> = {}) {
  ...
  Object.keys(intlPaths).forEach((path) => {
    ...
    schema
      .virtual(path)
      .get(function () {
        const locale = this.__locale || schemaDefaultLocales.get(schema) || defaultLocale
  ...
  schema.method('setAllTranslations', function (field: string, translations: Record<string, any>) {
    if (!intlPaths[field]) {
      throw new Error(`Field '${field}' is not internationalized`)
    }
  ...
  schema.static('getDefaultLocale', () => schemaDefaultLocales.get(schema) || defaultLocale)
  ...
  schema.pre('save', function (next) {
    ...
    const currentDefaultLocale = schemaDefaultLocales.get(schema) || defaultLocale

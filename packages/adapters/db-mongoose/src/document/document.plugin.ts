...
export class DocumentPluginService {
  ...
  private convertUniqueIndexes(schema: Schema): void {
    ...
    schema.eachPath((pathName, schemaType) => {
      ...
      if (hasUniqueOption || hasUniqueIndex) {
        this.logger.log(
          `Converting unique index on '${pathName}' to partial unique index for i18n support`,
        )
    ...
    const schemaIndexes = (schema as any)._indexes as
  ...
  private addMethods(schema: Schema): void {
    ...
    schema.static(
      'findByDocumentId',
      function (documentId: string, options: { locale?: string; status?: string } = {}) {
    ...
    schema.static(
      'findOneByDocumentId',
      function (documentId: string, locale: string, status: 'draft' | 'published') {

...
describe('NodemailerEmailAdapter', () => {
  ...
  it('should handle send failure gracefully', async () => {
    mocks.mockSendMail.mockImplementationOnce(() => Promise.reject(new Error('Connection refused')))
    const adapter = new NodemailerEmailAdapter(config)
    const result = await adapter.send({
      to: 'user@example.com',
      subject: 'Test',
  ...
  it('should include attachments in mail options', async () => {
    ...
    expect(mocks.mockSendMail).toHaveBeenCalledTimes(1)
    const callArgs = mocks.mockSendMail.mock.calls[0]?.[0] as Record<string, unknown>
  ...
  it('should handle verify failure', async () => {
    mocks.mockVerify.mockImplementationOnce(() => Promise.reject(new Error('Auth failed')))
    const adapter = new NodemailerEmailAdapter(config)
    const result = await adapter.verify()

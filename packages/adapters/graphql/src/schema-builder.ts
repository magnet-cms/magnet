...
const CONTEXT_PARAM_TYPES = new Set([
	'REQUEST',
	'RESPONSE',
	'NEXT',
	'HEADERS',
...
function resolveControllerInstance(
	...
	return (
		instances.get(controllerName) ??
		instances.get(controllerName.replace('controller', ''))
	)
}
...
function buildControllerGraphQLFields(
	...
	for (const controller of controllers) {
		...
		for (const method of controller.methods) {
			...
			const originalMethodName = Object.getOwnPropertyNames(
				Object.getPrototypeOf(instance),
			).find((m) => m.toLowerCase() === method.name)
			...
			const field: GraphQLFieldConfig<unknown, AnyContext> = {
				...
				resolve: async (
					...
					if (typeof fn !== 'function') {
						throw new Error(
							`Method ${originalMethodName} not found on controller`,
						)
					}
...
export function buildGraphQLSchema(
	...
	if (controllers && controllerInstances) {
		const { queries, mutations } = buildControllerGraphQLFields(
			controllers,
			controllerInstances,
		)

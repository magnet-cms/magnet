...
export function generateSchema(schemaClass: Type): {
  ...
  const table = pgTable(tableName, columns, (table) => {
    ...
    if (hasI18n || hasIntlProps) {
      ...
      tableIndexes.documentLocaleIdx = index(`${tableName}_document_locale_idx`).on(
        t.documentId,
        t.locale,
      )
    ...
    propsMetadata.forEach((options, propertyKey) => {
      if (options.unique) {
        ...
        if (hasI18n || hasIntlProps) {
          ...
          const whereCondition = and(eq(t.locale, 'en'), eq(t.status, 'draft')) as any
          ...
          tableIndexes[`${columnName}Unique`] = uniqueIndex(`${tableName}_${columnName}_unique`).on(
            t[columnName],
          )
        }
      }

 ...
 */
export function applyDocumentColumns(options: DocumentColumnOptions): Record<string, any> {
  ...
  if (options.hasI18n || options.hasVersioning) {
    ...
    columns.locale = varchar('locale', { length: 10 }).notNull().default(DEFAULT_LOCALE)

    // status for versioning/publishing workflow
    columns.status = varchar('status', { length: 20 }).notNull().default(DOCUMENT_STATUS.DRAFT)

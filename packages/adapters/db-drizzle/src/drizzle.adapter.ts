...
export class DrizzleDatabaseAdapter extends DatabaseAdapter {
  ...
  private pool: Pool | { end?: () => Promise<void>; close?: () => void } | null = null
  private options: DBConfig | null = null
  private schemaRegistry: Map<string, { table: PgTable; tableName: string }> = new Map()
  ...
  private async runAutoMigration(config: DrizzleConfig): Promise<void> {
    ...
    const migrationDb: MigrationDb = {
      execute: async (sql: string) =>
        (this.db as unknown as { execute: (s: unknown) => Promise<unknown> }).execute(sql),
    }
    ...
    try {
      await auto.run(config.dialect ?? 'postgresql', migrationConfig, migrationConfig.directory)
    } catch (err) {
  ...
  private async createTables(): Promise<void> {
    ...
    if (hasErrors) {
      logger.warn('Some tables could not be created. The application may not work correctly.')
    }
  }
  ...
  private async createTableFromConfig(
    ...
    for (const col of columnsArray) {
      ...
      if (typeof column.getSQLType === 'function') {
        ...
        sqlType =
          this.inferSQLType(column, dialect) || (dialect === 'mysql' ? 'VARCHAR(255)' : 'TEXT')
      }
      ...
      if (column.default !== undefined) {
        ...
        const defaultSQL = this.formatDefaultValue(defaultValue, sqlType, dialect)
        if (defaultSQL) {
    ...
    for (const col of columnsArray) {
      ...
      if (dialect === 'postgresql') {
        ...
        const addColSQL = sql.raw(`ALTER TABLE ${q(tableName)} ADD COLUMN ${colDefinition}`)
    ...
    if (addColumnClauses.length > 0 && dialect === 'postgresql') {
      const batchSQL = sql.raw(`ALTER TABLE ${q(tableName)} ${addColumnClauses.join(', ')}`)
    ...
    if (indexes) {
      for (const index of Object.values(indexes)) {
        ...
        const indexName =
          (indexConfig.name as string) || `${tableName}_${Math.random().toString(36).slice(2)}_idx`

        let indexColumns: string[] = []
        if (Array.isArray(indexConfig.columns)) {
          indexColumns = indexConfig.columns as string[]
        } else if (indexConfig.columns && typeof indexConfig.columns === 'object') {
  ...
  private mapSQLTypeForDialect(pgType: string, dialect: string): string {
    ...
    if (dialect === 'mysql') {
      ...
      if (upper.includes('DOUBLE PRECISION') || upper.includes('NUMERIC')) return 'DOUBLE'
    ...
    if (dialect === 'sqlite') {
      ...
      if (upper.includes('DOUBLE PRECISION') || upper.includes('NUMERIC')) return 'REAL'
   ...
   */
  private inferSQLType(column: Record<string, unknown>, _dialect: string): string | null {
    const columnStr = String(column)
    if (columnStr.includes('uuid')) return 'UUID'
    if (columnStr.includes('timestamp')) return 'TIMESTAMP'
    if (columnStr.includes('double') || columnStr.includes('numeric')) return 'DOUBLE PRECISION'
    if (columnStr.includes('boolean')) return 'BOOLEAN'
    if (columnStr.includes('jsonb') || columnStr.includes('json')) return 'JSONB'
    if (columnStr.includes('integer') || columnStr.includes('int')) return 'INTEGER'
    return null
  }
        ...
        providers: [
          {
            provide: DRIZZLE_DB,
            useFactory: async () => {
              const { db, pool } = await this.createConnectionAsync(drizzleConfig)
              this.db = db
              this.pool = pool
              return db
            },
  ...
  forFeature(schemas: Type | Type[]): DynamicModule {
    ...
    const providers = schemaArray.map((schemaClass) => {
      ...
      return {
        ...
        useFactory: async () => {
          if (!this.db) {
            throw new Error('Drizzle database not initialized. Call connect() first.')
          }
  ...
  private createLazyModel<T>(
    ...
    return class LazyDrizzleModelAdapter {
      private _instance: InstanceType<ReturnType<typeof createModel<T>>> | null = null
      private _initPromise: Promise<void> | null = null
      private _factory = factory
      currentLocale?: string
      currentVersion?: string

      private async _ensureInstance(): Promise<InstanceType<ReturnType<typeof createModel<T>>>> {
        ...
        this._initPromise = (async () => {
          ...
          const AdapterClass = createModel<T>(data.db, data.table, data.schemaClass)
      ...
      async create(
        data: Parameters<InstanceType<ReturnType<typeof createModel<T>>>['create']>[0],
        options?: Parameters<InstanceType<ReturnType<typeof createModel<T>>>['create']>[1],
      ) {
      ...
      async findOne(
        query: Parameters<InstanceType<ReturnType<typeof createModel<T>>>['findOne']>[0],
      ) {
        const inst = await this._ensureInstance()
        return inst.findOne(query)
      }
      async findMany(
        query: Parameters<InstanceType<ReturnType<typeof createModel<T>>>['findMany']>[0],
      ) {
        const inst = await this._ensureInstance()
        return inst.findMany(query)
      }
      async update(
        query: Parameters<InstanceType<ReturnType<typeof createModel<T>>>['update']>[0],
        data: Parameters<InstanceType<ReturnType<typeof createModel<T>>>['update']>[1],
        options?: Parameters<InstanceType<ReturnType<typeof createModel<T>>>['update']>[2],
      ) {
        const inst = await this._ensureInstance()
        return inst.update(query, data, options)
      }
      async delete(
        query: Parameters<InstanceType<ReturnType<typeof createModel<T>>>['delete']>[0],
      ) {
        const inst = await this._ensureInstance()
  ...
  getCapabilities(): AdapterCapabilities {
    return {
      ...
      features: ['transactions', 'json-queries', 'full-text-search', 'migrations'],
  ...
  private createConnection(config: DrizzleConfig): {
    ...
    switch (dialect) {
      ...
      case 'mysql': {
        // prettier-ignore -- multiline typeof import breaks DTS build
  ...
  async onModuleDestroy(): Promise<void> {
    if (this.pool) {
      if (typeof (this.pool as { end?: () => Promise<void> }).end === 'function') {
        await (this.pool as { end: () => Promise<void> }).end()
      } else if (typeof (this.pool as { close?: () => void }).close === 'function') {

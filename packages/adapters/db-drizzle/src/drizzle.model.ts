 ...
 * @internal
 */
export function createModel<T>(db: DrizzleDB, table: PgTable, schemaClass: Type) {
  return class DrizzleModelAdapter extends Model<T> {
     ...
     * Drizzle delegates versioning to HistoryService
     */
    createVersion(_documentId: string, _data: Partial<T>): Promise<VersionDocument | null> {
    ...
    async create(
      ...
      try {
        ...
        if (!result || result.length === 0) {
          throw new DatabaseError('Insert failed - no result returned', undefined, {
     ...
     */
    async findOne(query: Partial<BaseSchema<T>>): Promise<BaseSchema<T> | null> {
      try {
        const dynamicTable = this._table as DynamicTable
        const dbWithSelect = this._db as DrizzleDB & { select: Function }
     ...
     */
    async findVersions(_documentId: string): Promise<VersionDocument[]> {
      // Versions are managed by HistoryService in core
      return []
    }
     ...
     */
    async findVersionById(_versionId: string): Promise<VersionDocument | null> {
      // Versions are managed by HistoryService in core
      return null
    }
     ...
     * Restore a version
     */
    async restoreVersion(_versionId: string): Promise<BaseSchema<T> | null> {
      // Versions are managed by HistoryService in core
      return null
    }
    ...
    native(): NativeAccess<T> {
      ...
      return {
        raw: { db, table },
        adapterName: 'drizzle',
        async rawQuery<R = unknown>(query: string, _params?: unknown[]): Promise<R> {
    ...
    getMetadata(): SchemaMetadata {
      ...
      for (const [key, column] of Object.entries(dynamicTable)) {
        ...
        if (typeof column !== 'object' || column === null || !('dataType' in column)) {
          continue
        }
    ...
    _buildConditions(query: Partial<BaseSchema<T>>): DrizzleCondition[] {
      ...
      for (const [key, value] of Object.entries(query)) {
        ...
        if (key === '$or' && Array.isArray(value)) {
          const orConds = value
            .map((f) => {
              const conds = this._buildConditions(f as Partial<BaseSchema<T>>).filter(
     ...
     * @internal
     */
    _mapOperator(column: DynamicTableColumn, operator: string, value: unknown): DrizzleCondition {
      switch (operator) {
        case '$eq':
      ...
      for (const [key, value] of Object.entries(row)) {
        ...
        if (
          ...
          if (value instanceof Date) {
            ...
            result[camelKey] = Number.isNaN(dateValue.getTime()) ? value : dateValue
          } else {
    ...
    _prepareData(data: Record<string, unknown>, isCreate = false): Record<string, unknown> {
      const result: Record<string, unknown> = {}
      ...
      if (isCreate) {
        result.createdAt = data.createdAt instanceof Date ? data.createdAt : new Date()
      }

      // Always set updatedAt on every write
      result.updatedAt = data.updatedAt instanceof Date ? data.updatedAt : new Date()

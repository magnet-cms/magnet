...
const DANGEROUS_PATTERNS: Array<{
...
}> = [
  {
    ...
    message: (m) => `DROP TABLE "${m}" will permanently delete all data in this table`,
  },
  {
  ...
  },
  {
    ...
    message: (m) => `Type change in "${m}" may cause data loss or conversion errors`,
  },
]
...
export class SchemaDiff {
   ...
   */
  async diff(dialect: MigrationDialect, prevSnapshot?: SnapshotJSON): Promise<SchemaDiffResult> {
    ...
    const upSQL = await this.bridge.generateSQL(prev, currentSnapshot, undefined, dialect)

...
describe('PostgreSQL SQL generator', () => {
  ...
  it('dropColumn generates correct SQL', () => {
    expect(pg.dropColumn('users', 'email')).toBe('ALTER TABLE "users" DROP COLUMN "email"')
  })
  ...
  it('createIndex generates correct SQL', () => {
    expect(pg.createIndex(idx)).toContain('INDEX "users_email_idx" ON "users" ("email")')
  })
...
describe('MySQL SQL generator', () => {
  it('createTable uses backtick quoting', () => {
    const sql = mysql.createTable('users', [{ name: 'id', type: 'VARCHAR(36)', primaryKey: true }])
    expect(sql).toContain('CREATE TABLE `users`')
  ...
  it('dropColumn generates correct SQL', () => {
    expect(mysql.dropColumn('users', 'email')).toBe('ALTER TABLE `users` DROP COLUMN `email`')
  })
...
describe('SQLite SQL generator', () => {
  it('createTable generates correct SQL', () => {
    const sql = sqlite.createTable('users', [{ name: 'id', type: 'TEXT', primaryKey: true }])
    expect(sql).toContain('CREATE TABLE "users"')
  ...
  it('dropIndex generates correct SQL', () => {
    expect(sqlite.dropIndex('users_email_idx')).toBe('DROP INDEX "users_email_idx"')
  })
})

...
export class AutoMigration {
   ...
   */
  async run(dialect: MigrationDialect, config: MigrationConfig, directory: string): Promise<void> {
    ...
    const { filename } = await this.gen.writeMigrationFile(directory, 'auto_migration', content)

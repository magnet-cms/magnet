...
export class MigrationRunner {
   ...
   */
  async up(migrations: Migration[], options?: { to?: string }): Promise<MigrationResult> {
    ...
    try {
      ...
      for (const migration of migrations) {
        ...
        if (record) {
          const checksum = MigrationHistory.calculateChecksum(migration.up, migration.down)
          if (checksum !== record.checksum) {
            throw new MigrationChecksumError(migration.id, record.checksum, checksum)
          }
        }
      }
      ...
      for (const migration of pending) {
        ...
        const record: MigrationHistoryRecord = {
          ...
          appliedAt: new Date(),
          checksum: MigrationHistory.calculateChecksum(migration.up, migration.down),
        }
   ...
   */
  async down(migrations: Migration[], options?: { to?: string }): Promise<void> {
    await this.history.ensureTable()
    await this.lock.ensureTable()
    await this.lock.acquire()

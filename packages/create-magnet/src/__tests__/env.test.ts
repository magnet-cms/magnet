...
describe('generateDotEnv — common fields', () => {
  ...
  it('includes JWT_SECRET', () => {
    expect(output).toContain('JWT_SECRET=magnet-local-dev-secret-change-in-production')
  })
...
describe('generateDotEnv — Mongoose', () => {
  ...
  it('has MongoDB URI with magnet credentials', () => {
    expect(output).toContain('MONGODB_URI=mongodb://magnet:magnet@localhost:27017/my-app')
  })
...
describe('generateDotEnv — Drizzle Neon', () => {
  ...
  it('has PostgreSQL connection string with magnet credentials', () => {
    expect(output).toContain('DATABASE_URL=postgresql://magnet:magnet@localhost:5432/my-app')
  })

...
export function generatePackageJson(config: ProjectConfig): string {
  ...
  if (database === 'mongoose') {
    ...
    if (database === 'drizzle-neon') {
      dependencies['@neondatabase/serverless'] = PACKAGE_VERSIONS['@neondatabase/serverless']
      ...
      dependencies['@supabase/supabase-js'] = PACKAGE_VERSIONS['@supabase/supabase-js']
  ...
  if (plugins.includes('seo')) {
    dependencies['@magnet-cms/plugin-seo'] = PACKAGE_VERSIONS['@magnet-cms/plugin-seo']
  }

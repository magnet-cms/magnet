...
export function generateAppModule(config: ProjectConfig): string {
  ...
  if (plugins.includes('playground')) {
    imports.push("import { PlaygroundPlugin } from '@magnet-cms/plugin-playground'")
  }
...
function getDatabaseImports(config: ProjectConfig): string[] {
  if (config.database === 'mongoose') {
    return ["import { MongooseDatabaseAdapter } from '@magnet-cms/adapter-db-mongoose'"]
  }
  return ["import { DrizzleDatabaseAdapter } from '@magnet-cms/adapter-db-drizzle'"]
}

function getStorageImports(config: ProjectConfig): string[] {
  switch (config.storage) {
    case 's3':
      return ["import { S3StorageAdapter } from '@magnet-cms/adapter-storage-s3'"]
    case 'r2':
      return ["import { R2StorageAdapter } from '@magnet-cms/adapter-storage-r2'"]
    case 'supabase':
      return ["import { SupabaseStorageAdapter } from '@magnet-cms/adapter-storage-supabase'"]
    default:
      return []
  }
}
...
function getAuthImports(config: ProjectConfig): string[] {
  if (config.database === 'drizzle-supabase') {
    return ["import { SupabaseAuthAdapter } from '@magnet-cms/adapter-auth-supabase'"]
  }
  return []
}
...
function getVaultImports(config: ProjectConfig): string[] {
  if (config.database === 'drizzle-supabase') {
    return ["import { SupabaseVaultAdapter } from '@magnet-cms/adapter-vault-supabase'"]
  }
  return []
}
...
function generateProviders(config: ProjectConfig): string {
  ...
  if (database === 'mongoose') {
    ...
    lines.push(`\t\t\t\t\tmode: process.env.NODE_ENV === 'production' ? 'manual' : 'auto',`)
...
function generateGlobalOptions(_config: ProjectConfig): string {
  return ', { admin: true }'
}

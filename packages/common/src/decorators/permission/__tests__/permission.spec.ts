...
describe('RequirePermission', () => {
  it('stores full PermissionOptions on descriptor.value', () => {
    ...
    const descriptor = Object.getOwnPropertyDescriptor(PermA.prototype, 'method')!
    RequirePermission({
    ...
    const metadata = Reflect.getMetadata(PERMISSION_METADATA_KEY, descriptor.value)
  ...
  it('stores metadata via class decorator syntax', () => {
    ...
    const descriptor = Object.getOwnPropertyDescriptor(PermB.prototype, 'method')!
    const metadata = Reflect.getMetadata(PERMISSION_METADATA_KEY, descriptor.value)
    expect(metadata).toBeDefined()
  ...
  it('returns the original descriptor', () => {
    ...
    const descriptor = Object.getOwnPropertyDescriptor(PermC.prototype, 'method')!
...
describe('HasPermission', () => {
  it('auto-generates name from last segment of permission string', () => {
    ...
    const descriptor = Object.getOwnPropertyDescriptor(HasPermD.prototype, 'method')!
  ...
  it('auto-generates description from permission string', () => {
    ...
    const descriptor = Object.getOwnPropertyDescriptor(HasPermE.prototype, 'method')!
    HasPermission('content.posts.create')(HasPermE.prototype, 'method', descriptor)
    const metadata = Reflect.getMetadata(PERMISSION_METADATA_KEY, descriptor.value)
    expect(metadata.description).toBe('Requires content.posts.create permission')
  })
  ...
  it('uses full permission as name when no dots present', () => {
    ...
    const descriptor = Object.getOwnPropertyDescriptor(HasPermF.prototype, 'method')!
...
describe('PermissionMeta', () => {
  it('stores partial options under PERMISSION_OPTIONS_METADATA_KEY', () => {
    ...
    const descriptor = Object.getOwnPropertyDescriptor(PermMetaG.prototype, 'method')!
    PermissionMeta({ group: 'Admin' })(PermMetaG.prototype, 'method', descriptor)
    const metadata = Reflect.getMetadata(PERMISSION_OPTIONS_METADATA_KEY, descriptor.value)
    expect(metadata).toBeDefined()
  ...
  it('stores description-only options', () => {
    ...
    const descriptor = Object.getOwnPropertyDescriptor(PermMetaH.prototype, 'method')!
    PermissionMeta({ description: 'Extra description' })(PermMetaH.prototype, 'method', descriptor)
    const metadata = Reflect.getMetadata(PERMISSION_OPTIONS_METADATA_KEY, descriptor.value)

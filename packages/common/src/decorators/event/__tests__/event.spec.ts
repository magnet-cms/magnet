...
describe('OnEvent', () => {
  ...
  it('stores custom priority option', () => {
    ...
    OnEvent('content.updated', { priority: 10 })(EvB.prototype, 'method', descriptor)
    const metadata = Reflect.getMetadata(EVENT_HANDLER_METADATA, descriptor.value)
  ...
  it('stores combined options (priority + async)', () => {
    ...
    OnEvent('content.deleted', { priority: 50, async: true })(EvD.prototype, 'method', descriptor)
    const metadata = Reflect.getMetadata(EVENT_HANDLER_METADATA, descriptor.value)
  ...
  it('stores metadata via class decorator syntax', () => {
    ...
    const descriptor = Object.getOwnPropertyDescriptor(EvE.prototype, 'handler')!
    const metadata = Reflect.getMetadata(EVENT_HANDLER_METADATA, descriptor.value)
  ...
  it('returns the original descriptor', () => {
    ...
    const result = OnEvent('content.created')(EvF.prototype, 'method', descriptor)

...
describe('mapFieldTypeToUI', () => {
	...
	it('maps boolean with style:checkbox → type:checkbox', () => {
		expect(
			mapFieldTypeToUI('boolean', { style: 'checkbox' } as BooleanFieldOptions)
				.type,
		).toBe('checkbox')
	})
	...
	it('maps select → type:select with converted options', () => {
		...
		const opts = (result as { options?: { key: string; value: string }[] })
			.options!
	...
	it('maps array → type:array', () => {
		expect(
			mapFieldTypeToUI('array', { of: { type: 'text' } } as ArrayFieldOptions)
				.type,
		).toBe('array')
	})

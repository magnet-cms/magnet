...
import { DESIGN_TYPE, FIELD_METADATA_KEY, PROP_METADATA_KEY, UI_METADATA_KEY } from '~/constants'
import type { BaseFieldOptions, FieldMetadata, FieldTypeId } from '~/types/field.types'
...
export function createFieldDecorator<T extends BaseFieldOptions>(
  ...
  return (options?: T): PropertyDecorator => {
    ...
    return (target: object, propertyKey: string | symbol): void => {
      ...
      Reflect.defineMetadata(FIELD_METADATA_KEY, [...filteredFields, metadata], target.constructor)
...
function emitLegacyUIMetadata(
  ...
  const existingUI: UIFieldMetadataEntry[] = Reflect.getMetadata(UI_METADATA_KEY, target) ?? []
...
function applyAdapterProp(
  ...
  try {
    ...
    const { Prop } = requireDatabaseAdapterModule(adapter) as {
      Prop: (options: ReturnType<typeof mapFieldTypeToProp>) => PropertyDecorator
    }

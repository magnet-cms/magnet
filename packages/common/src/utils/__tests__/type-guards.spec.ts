...
describe('isCastError', () => {
	it('returns true for CastError-shaped objects', () => {
		expect(isCastError({ name: 'CastError', path: '_id', value: 'bad' })).toBe(
			true,
		)
	})
...
describe('isDuplicateKeyError', () => {
	it('returns true for code 11000', () => {
		expect(isDuplicateKeyError({ code: 11000, keyValue: { email: 'x' } })).toBe(
			true,
		)
	})
...
describe('isValidationError', () => {
	it('returns true for ValidationError-shaped objects', () => {
		expect(isValidationError({ name: 'ValidationError', errors: {} })).toBe(
			true,
		)
	})
...
describe('isVersionDocument', () => {
	...
	it('returns false when required fields are missing', () => {
		expect(
			isVersionDocument({ ...validVersionDoc, documentId: undefined }),
		).toBe(false)
		expect(isVersionDocument({ ...validVersionDoc, versionId: 123 })).toBe(
			false,
		)
		expect(
			isVersionDocument({ ...validVersionDoc, data: 'not-an-object' }),
		).toBe(false)
	})
})

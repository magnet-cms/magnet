...
export const EMAIL_TEMPLATE_KEYS = {
	...
	versions: (id: string) =>
		[...EMAIL_TEMPLATE_KEYS.all, 'versions', id] as const,
	bySlug: (slug: string) =>
		[...EMAIL_TEMPLATE_KEYS.all, 'by-slug', slug] as const,
}
...
export const useEmailTemplateList = (filters?: EmailTemplateListFilters) => {
	...
	return useQuery<EmailTemplate[], Error>({
		...
		queryFn: () =>
			adapter.request<EmailTemplate[]>(
				`/email-templates${query ? `?${query}` : ''}`,
			),
	})
}
...
export const useEmailTemplatesBySlug = (slug: string) => {
	...
	return useQuery<EmailTemplate[], Error>({
		...
		queryFn: () =>
			adapter.request<EmailTemplate[]>(`/email-templates/by-slug/${slug}`),
		enabled: !!slug,
	})
}
...
export const useEmailTemplateVersions = (id: string) => {
	...
	return useQuery<EmailTemplateVersion[], Error>({
		...
		queryFn: () =>
			adapter.request<EmailTemplateVersion[]>(
...
export const useEmailTemplateUpdate = () => {
	...
	return useMutation<
		EmailTemplate,
		Error,
		{ id: string; data: UpdateEmailTemplateData }
	>({
		mutationFn: ({ id, data }) =>

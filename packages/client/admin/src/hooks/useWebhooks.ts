...
export const WEBHOOK_KEYS = {
  ...
  deliveries: (id: string, page?: number) => [...WEBHOOK_KEYS.all, 'deliveries', id, page] as const,
}
...
export const useWebhookDeliveries = (webhookId: string, page = 1, limit = 20) => {
  const adapter = useAdapter()
...
export const useWebhookUpdate = () => {
  ...
  return useMutation<WebhookConfig, Error, { id: string; data: UpdateWebhookData }>({
    mutationFn: ({ id, data }) =>

...
export const useMediaUpload = () => {
  ...
  return useMutation<MediaItem, Error, { file: File; options?: MediaUploadOptions }>({
...
export const useMediaUploadMultiple = () => {
  ...
  return useMutation<MediaItem[], Error, { files: File[]; options?: MediaUploadOptions }>({
    mutationFn: ({ files, options }) => adapter.media.uploadMultiple(files, options),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['media'] })
    },
  })
...
export const useMediaCreateFolder = () => {
  ...
  return useMutation<MediaFolder, Error, { name: string; parentPath?: string }>({
    mutationFn: ({ name, parentPath }) => adapter.media.createFolder(name, parentPath),
    onSuccess: () => {
...
export const useMediaUrl = () => {
  ...
  return {
     ...
     */
    getUrl: (id: string, transform?: TransformOptions) => adapter.media.getUrl(id, transform),
    /**

 ...
 */
export const useContentVersions = (schema: string, documentId: string, locale?: string) => {
  const adapter = useAdapter()
...
export const useContentCreate = <T extends Record<string, unknown>>() => {
  ...
  >({
    mutationFn: ({ schema, data, options }) => adapter.content.create<T>(schema, data, options),
    onSuccess: (_, { schema }) => {
...
export const useContentCreateEmpty = () => {
  ...
  >({
    mutationFn: ({ schema, options }) => adapter.content.createEmpty(schema, options),
    onSuccess: (_, { schema }) => {
      queryClient.invalidateQueries({ queryKey: CONTENT_KEYS.list(schema) })
    },
  })
...
export const useContentDelete = () => {
  ...
  return useMutation<void, Error, { schema: string; documentId: string }>({
    mutationFn: ({ schema, documentId }) => adapter.content.delete(schema, documentId),
    onSuccess: (_, { schema }) => {
      queryClient.invalidateQueries({ queryKey: CONTENT_KEYS.list(schema) })

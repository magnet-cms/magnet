...
import { ExternalLink, MoreHorizontal, Pencil, Plus, Send, Trash2 } from 'lucide-react'
...
export function WebhooksListingPage() {
      ...
      { id: webhook.id, data: { enabled: !webhook.enabled } },
      {
        onSuccess: () => {
          toast.success(
            intl.formatMessage(
              {
                id: webhook.enabled ? 'webhooks.toast.disabled' : 'webhooks.toast.enabled',
            ...
            intl.formatMessage(
              {
                ...
                defaultMessage: 'Test delivery successful ({statusCode}, {duration}ms)',
              },
              {
  ...
  return (
    <div className="p-6 space-y-6">
            ...
            {webhooks.map((webhook) => (
              <TableRow key={webhook.id}>
                <TableCell className="font-medium">
                  {webhook.name}
                  {webhook.description && (
                    <p className="text-xs text-muted-foreground mt-0.5">{webhook.description}</p>
                  )}
                </TableCell>
                <TableCell>
                ...
                <TableCell className="text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem onClick={() => setDeliveryWebhookId(webhook.id)}>
        ...
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p className="text-sm mt-1">
            {intl.formatMessage({
              ...
              defaultMessage: 'Create a webhook to start receiving event notifications via HTTP.',
            })}
          </p>
      ...
      <WebhookFormDialog open={formOpen} onOpenChange={setFormOpen} webhook={editingWebhook} />

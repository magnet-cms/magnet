import { Avatar, AvatarFallback, AvatarImage } from '@magnet-cms/ui/components/atoms'
      ...
      <div className="mt-6 flex items-center justify-between border-t border-white/10 pt-4">
        <div className="flex flex-col">
          <span className="text-[10px] font-normal uppercase tracking-wider text-white/40">
            {intl.formatMessage({
              id: 'auth.tripPreview.totalBudget',
        ...
        <button
          type="button"
          className="flex size-8 items-center justify-center rounded-full bg-white transition-transform hover:scale-105"
        >
        ...
        {activity.avatars && activity.avatars.length > 0 && (
          <div className="mt-1.5 flex items-center gap-2">
            <div className="flex -space-x-1.5">
              {activity.avatars.map((avatar) => (
                <Avatar key={avatar} className="size-4 ring-1 ring-black">
            ...
            {activity.othersCount && activity.othersCount > 0 && (
              <span className="text-[9px] text-white/40">
                {intl.formatMessage(
                  {
                    id: 'auth.tripPreview.othersCount',
                    defaultMessage: '+{count} others',
                  },
                  { count: activity.othersCount },
                )}
              </span>
            )}
          </div>

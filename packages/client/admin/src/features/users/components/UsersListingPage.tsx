...
const statusColors: Record<UserStatusKey, { bg: string; text: string; ring: string }> = {
  Active: {
...
function formatUserRelativeTime(date: Date, intl: ReturnType<typeof useAppIntl>): string {
  const now = new Date()
...
function transformUser(apiUser: ApiUser, intl: ReturnType<typeof useAppIntl>): User {
  const status: UserStatusKey =
    apiUser.isActive === false ? 'Inactive' : apiUser.lastLogin ? 'Active' : 'Pending'
...
function statusDisplayLabel(status: UserStatusKey, intl: ReturnType<typeof useAppIntl>): string {
  switch (status) {
    case 'Active':
      return intl.formatMessage({
...
export function UsersListingPage() {
  ...
  const columns: DataTableColumn<User>[] = [
    ...
    },
    {
      ...
      accessorKey: 'email',
      format: (value) => <div className="text-sm text-muted-foreground">{value as string}</div>,
    },
    {
      type: 'selector',
      header: intl.formatMessage({
  ...
  const filteredData = users.filter((user) => {
    ...
    const matchesStatus = statusFilter === '__all_status__' || user.status === statusFilter
              ...
              {intl.formatMessage(
                {
                  ...
                  defaultMessage: '{count, plural, one {# user} other {# users}} total.',
                },

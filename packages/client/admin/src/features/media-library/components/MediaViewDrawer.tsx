              ...
              {asset.type === 'image' && asset.url ? (
                ...
                (() => {
                  ...
                  return (
                    ...
                    >
                      <docIcon.icon
                        className={cn(isExpanded ? 'w-24 h-24' : 'w-16 h-16', docIcon.color)}
                      />
                      <span
                        className={cn(
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground mt-2 px-1">
              {intl.formatMessage({
                ...
                defaultMessage: 'Tip: Drag and drop a new file here to replace current version.',
              })}
            </p>
          </div>
            ...
            <div>
              ...
              <div className="flex items-center justify-between p-2 bg-muted/50 border border-border rounded-md">
                <div className="flex items-center gap-2 overflow-hidden">
                  <div className="w-4 h-4 text-muted-foreground shrink-0">📁</div>
                ...
                <div className="mt-2 p-2 bg-background border border-border rounded-md space-y-1">
                  <button
                    ...
                    className={cn(
                      ...
                      !asset.location || asset.location === 'Root' ? 'bg-muted font-medium' : '',
                    )}
                    onClick={() => {
                      onMove(asset.id, '')
                  ...
                  {folders.map((folder) => (
                    <button
                      ...
                      className={cn(
                        'w-full text-left text-xs px-2 py-1.5 rounded hover:bg-muted transition-colors',
                        asset.location === folder.path ? 'bg-muted font-medium' : '',
                      )}
                      onClick={() => {
                  ...
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' && subfolderName.trim()) {
                        const parentPath =
                          asset.location && asset.location !== 'Root' ? asset.location : undefined
            ...
            <div>
              ...
              <dl className="grid grid-cols-2 gap-x-4 gap-y-4">
                <div>
                  ...
                  <dd className="mt-0.5 text-xs font-medium text-foreground">{asset.size}</dd>
                </div>
                <div>
                  <dt className="text-[10px] text-muted-foreground uppercase tracking-wide">
                    {intl.formatMessage({

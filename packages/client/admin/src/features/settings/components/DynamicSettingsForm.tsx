 ...
 */
export const DynamicSettingsForm = forwardRef<DynamicSettingsFormRef, DynamicSettingsFormProps>(
  function DynamicSettingsForm({ group }, ref) {
    ...
    const handleSave = async (data: SettingsFormValues): Promise<void> => {
      const payload = schemaFieldKeys
        ? Object.fromEntries(Object.entries(data).filter(([k]) => schemaFieldKeys.has(k)))
        : data
      ...
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(handleSave)} className="space-y-8">
          {parsedSchema.sections.map((section) => (
            <SettingsSectionCard key={section.name} section={section} disabled={isSaving} />
          ))}
        </form>
      </FormProvider>
    )

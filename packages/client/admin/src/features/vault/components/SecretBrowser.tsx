...
const columns: DataTableColumn<VaultSecretMeta>[] = [
  {
    type: 'custom',
    header: 'Name',
    cell: (row) => <span className="font-mono text-sm text-foreground">{row.original.name}</span>,
  },
  {
    type: 'custom',
    header: 'Description',
    cell: (row) => (
      <span className="text-sm text-muted-foreground">
        {row.original.description ?? <span className="text-muted-foreground/50">—</span>}
      </span>
    ),
  },
  {
  ...
  return (
    <>
      ...
      <SecretDrawer
        ...
        editKey={editingKey}
        initialValue={editingKey && secretDetail ? secretDetail.value : undefined}
        initialDescription={
          editingKey ? allSecrets.find((s) => s.name === editingKey)?.description : undefined
        }
      />
    </>
  )

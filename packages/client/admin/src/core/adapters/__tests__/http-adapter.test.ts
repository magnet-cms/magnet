...
function _makeAdapter(fetchImpl: typeof fetch) {
...
describe('createHttpAdapter error handling', () => {
  it('should use backend error message from JSON body when request fails', async () => {
    ...
    try {
      ...
      expect((err as Error).message).toBe('Password must be at least 8 characters')
    } finally {
  ...
  it('should handle array message fields from NestJS validation errors', async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(Error)
      expect((err as Error).message).toBe('email must be an email, password is too short')
    } finally {
      globalThis.fetch = orig
    }
  })

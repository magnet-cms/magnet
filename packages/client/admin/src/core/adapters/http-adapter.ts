...
export function createHttpAdapter(config: HttpAdapterConfig): MagnetApiAdapter {
   ...
   */
  async function request<T>(url: string, options?: ApiRequestConfig): Promise<T> {
  ...
  return {
    ...
    content: {
      async list<T = ContentData>(schema: string, options?: ContentQueryOptions): Promise<T[]> {
      ...
      async deleteLocale(
        ...
        return request<{ success: boolean }>(`/content/${schema}/${documentId}/locale/${locale}`, {
          method: 'DELETE',
        })
      },
      ...
      async getLocaleStatuses(
        ...
        return request<Record<string, LocaleStatus>>(`/content/${schema}/${documentId}/locales`)
      },
    ...
    settings: {
      ...
      async updateByGroup<T = ContentData>(group: string, data: Partial<T>): Promise<T> {
        return request<T>(`/settings/${group}`, {
          method: 'PUT',
          body: data,
        })
      ...
      async getLocales(): Promise<{
        ...
        const settings = await request<Record<string, unknown>>('/settings/general')
    ...
    history: {
      async getVersions(documentId: string, collection: string): Promise<VersionInfo[]> {
      ...
      async compareVersions(versionId1: string, versionId2: string): Promise<VersionDiff> {
        return request<VersionDiff>(`/history/compare/${versionId1}/${versionId2}`)
      },
    },

    notifications: {
      async list(options?: NotificationListOptions): Promise<PaginatedNotificationRecords> {
    ...
    activity: {
      ...
      async getByUser(userId: string, limit?: number): Promise<ActivityRecord[]> {
      ...
      async cleanup(retentionDays?: number): Promise<{ deleted: number }> {
        const url = buildUrl('/activity/cleanup', {
          retentionDays: retentionDays !== undefined ? String(retentionDays) : undefined,
        })
    ...
    playground: {
      ...
      async createSchema(data: PlaygroundCreateSchemaDto): Promise<PlaygroundCreateModuleResponse> {
      ...
      async previewCode(data: PlaygroundCreateSchemaDto): Promise<PlaygroundCodePreview> {
    ...
    media: {
      ...
      async upload(file: File, options?: MediaUploadOptions): Promise<MediaItem> {
        ...
        if (!res.ok) {
          const error = new HttpError(`Upload failed: ${res.statusText}`, res.status)
          onError?.(error)
          throw error
        }
      ...
      async uploadMultiple(files: File[], options?: MediaUploadOptions): Promise<MediaItem[]> {
        const formData = new FormData()
      ...
      async deleteMany(ids: string[]): Promise<{ deleted: number; failed: string[] }> {
        return request<{ deleted: number; failed: string[] }>('/media/delete-many', {
          method: 'POST',
          body: { ids },
        })
      },
      ...
      async createFolder(name: string, parentPath?: string): Promise<MediaFolder> {
      ...
      async deleteFolder(path: string): Promise<{ success: boolean }> {
        return request<{ success: boolean }>(`/media/folders/${encodeURIComponent(path)}`, {
          method: 'DELETE',
        })
      },

...
import { PluginRegistryProvider, usePluginRegistry } from '~/core/plugins/PluginRegistry'
// Import layouts
...
import { EmailTemplateEditorPage, EmailTemplatesListingPage } from '~/features/email-templates'
...
function SetupPage() {
  ...
  const { data: generalSettings } = useSettingData<Record<string, unknown>>('general')
  const { mutate: saveSettings, isPending } = useSettingMutation<Record<string, unknown>>('general')
  ...
  const savedBaseUrl = ((generalSettings?.baseUrl as string) ?? '').replace(/^https?:\/\//, '')
...
function SchemaFormPageNewWrapper() {
  const { schema = '' } = useParams<{ schema: string }>()
  const schemaNames = names(schema)
  return <SchemaFormPage schema={schema} schemaDisplayName={schemaNames.title} entryId="" />
}

...
export const fieldRenderer: Record<UITypes, (prop: SchemaProperty) => ReactElement> = {
  ...
  text: (prop) => (
    <RHFText
      ...
      description={prop.ui?.description}
      placeholder={
        prop.ui?.placeholder || `Enter ${capitalize(prop.ui?.label || prop.name).toLowerCase()}`
      }
    />
  ),
  textarea: (prop) => (
  ...
  select: (prop) => {
    ...
    if (ui.multi) {
      return (
        <RHFMultiSelect
          ...
          placeholder={
            ui.placeholder || `Select ${capitalize(ui.label || prop.name).toLowerCase()}`
          }
          options={opts}
        />
      )
    ...
    return (
      <RHFSelect
        ...
        label={capitalize(ui.label || prop.name)}
        description={ui.description}
        placeholder={ui.placeholder || `Select ${capitalize(ui.label || prop.name).toLowerCase()}`}
        options={opts}
      />
    )
  },

...
export const buildFormSchema = (
  ...
  > = {
    ...
    regex: (field, constraints) =>
      field instanceof z.ZodString ? field.regex(new RegExp(constraints[0])) : field,
    isNotEmpty: (field) => (field instanceof z.ZodString ? field.min(1) : field),
    isPhoneNumber: (field) =>
      ...
      if (prop.validations) {
        prop.validations.forEach((validation) => {
          if (validationMappings[validation.name]) {
            ...
            if (validationFn) {
              field = validationFn(field, (validation.constraints as unknown[]) || [])
            }
          }
        })
      }

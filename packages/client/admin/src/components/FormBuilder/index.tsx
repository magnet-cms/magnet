...
import { SchemaMetadata, SchemaProperty, UISide, UITab } from '@magnet-cms/common'
import {
  FormProvider,
  RHFText,
  Tabs,
...
const normalizeInitialValues = (
  ...
  schema.properties
    .filter((prop) => prop.ui?.type === 'relationship' || prop.ui?.type === 'upload')
    .forEach((prop) => {
      ...
      if (prop.isArray && Array.isArray(value)) {
        ...
        if (value && typeof value === 'object' && 'id' in value) {
          normalized[prop.name] = typeof value.id === 'string' ? value.id : String(value.id)
...
export const FormBuilder = <T extends Record<string, unknown>>({
  ...
  const normalizedInitialValues = useMemo(
    () => normalizeInitialValues(initialValues as Record<string, unknown> | undefined, schema),
  ...
  const previousNormalizedValuesRef = useRef<Record<string, unknown> | undefined>(
  ...
  const [activeTab, setActiveTab] = useState<string>(tabs.length && tabs[0] ? tabs[0] : '')
  ...
  const hasSidebar = sidePanelFields.length > 0 || relationshipFields.length > 0 || metadata
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            {tabs.map((tab) => (
              <TabsContent key={tab} value={tab}>
                <div className="space-y-4 pt-6">{renderMainRows(groupedProperties[tab] || [])}</div>
              </TabsContent>
            ))}
          </Tabs>
        </div>
    ...
    return (
      ...
      >
        <fieldset disabled={disabled} className="contents">
          <div className="flex-1 min-w-0 overflow-y-auto p-8">{mainFormContent}</div>
          {renderSidebar({
            relationshipFields,
            sidePanelFields,
            renderField,
    ...
    <FormProvider onSubmit={submitHandler} {...methods}>
      <fieldset disabled={disabled} className="contents">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          {/* Main Form Column */}
          <div className={hasSidebar ? 'lg:col-span-8' : 'lg:col-span-12'}>{mainFormContent}</div>
                ...
                <div className="border-t border-border pt-6 space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium">{formatDate(metadata.publishedAt)}</span>
                    </div>
                  </div>
                </div>
              )}

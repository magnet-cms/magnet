...
export const ContentHeader = ({
  ...
  const getActiveTab = () => {
    ...
    const activeTab = tabs.find((tab) => {
      if (tab.to === '') {
        ...
        return !tabs.some((t) => t.to !== '' && currentPath.endsWith(`/${t.to}`))
      }
  ...
  const formatAutoSaveStatus = () => {
    ...
    if (lastEdited) {
      return (
        <span className="text-xs text-muted-foreground">Saved {formatLastEdited(lastEdited)}</span>
      )
    }
    return null
  }
  ...
  const statusContent = (status || lastEdited || autoSaveStatus) && (
    <>
      {status && (
        <Badge variant="outline" className="gap-1.5 bg-muted/50 border-border text-xs font-medium">
          <div
  ...
  return (
    <>
      ...
      {portalContainer && statusContent && createPortal(statusContent, portalContainer)}
      ...
      <header className="border-b border-border bg-background">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between px-6 py-4 gap-4">
          ...
          <div className="flex items-center gap-3">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    ...
                    >
                      ...
                      <span>{currentLocaleName || localeProps.currentLocale}</span>
                  ...
                  <DropdownMenuContent align="end" className="w-48">
                    {localeProps.locales.map((locale) => {
                      ...
                      const hasContent = status?.hasDraft || status?.hasPublished
                      return (
                        <DropdownMenuItem
                          key={locale.code}
                          onClick={() => localeProps.onLocaleChange(locale.code)}
                          disabled={!hasContent && locale.code !== localeProps.currentLocale}
                          className="justify-between"
                        >
                          <span
                            className={
                              locale.code === localeProps.currentLocale ? 'font-medium' : ''
                            }
                          >
                            {locale.name}
                          </span>
                      ...
                      <>
                        ...
                        {localeProps.locales
                          ...
                          .map((locale) => (
                            <DropdownMenuItem
                              key={`add-${locale.code}`}
                              onClick={() => localeProps.onAddLocale?.(locale.code)}
                            >
                              + {locale.name}
                            </DropdownMenuItem>
                          ))}
            ...
            {onPublish && (
              <Button size="sm" onClick={onPublish} disabled={isPublishing} variant="default">

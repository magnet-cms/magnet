...
export function AuthedLayout({ children, header, sidebar }: AuthedLayoutProps) {
  ...
  const pluginNavItems = useMemo(
    () =>
      pluginSidebarItems
        ...
        .sort((a, b) => {
          const orderA = pluginSidebarItems.find((p) => p.title === a.title)?.order ?? 50
          const orderB = pluginSidebarItems.find((p) => p.title === b.title)?.order ?? 50
  ...
  const activeSidebarConfig = useMemo(() => {
    const isContentManagerPage = location.pathname.startsWith('/content-manager/')
    ...
    const navMain = (defaultSidebarConfig.navMain ?? []).map((item) => {
      if (item.title === 'Content Manager' && item.items) {
        ...
        return {
          ...item,
          isActive: isContentManagerPage || location.pathname === '/content-manager',
          items,
        }
      }
      return {
    ...
    const navSecurity = (defaultSidebarConfig.navSecurity ?? []).map((item) => ({
      ...item,
      isActive: isNavItemActive(item.url, location.pathname),
    }))

    // Process navDeveloper items for active state
    const navDeveloper = (defaultSidebarConfig.navDeveloper ?? []).map((item) => ({
      ...item,
      isActive: isNavItemActive(item.url, location.pathname),
    }))

    // Process navSecondary items for active state
    const navSecondary = (defaultSidebarConfig.navSecondary ?? []).map((item) => ({
      ...item,
      isActive: isNavItemActive(item.url, location.pathname),
    }))

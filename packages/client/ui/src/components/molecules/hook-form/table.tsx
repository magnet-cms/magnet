        ...
        const rowActions: DataTableRowAction<T>[] = [
          {
            label: 'Edit',
            onSelect: (row) => {
              const index = items.findIndex((item) => getRowId(item) === getRowId(row))
              if (index !== -1) {
              ...
              <DialogContent>
                ...
                <div className="space-y-4 py-4">
                  {columns.map((col) => {
                    ...
                    if (col.key === 'isDefault' || col.key === 'isLocal' || col.key === 'id') {
                      return null
                    }

                    // Check if this field should be read-only for the current item
                    const isFieldReadOnly =
                      Boolean(editingItem?.[readOnlyKey]) && readOnlyFields.includes(col.key)
                      ...
                      <div key={col.key} className="space-y-2">
                        ...
                        {col.key.toLowerCase().includes('description') ? (
                          ...
                          <Input
                            id={col.key}
                            type={
                              col.key.toLowerCase().includes('connection') ? 'password' : 'text'
                            }
                            value={formData[col.key] || ''}
                            onChange={(e) =>
                              setFormData((prev) => ({

...
export function TimelineItem({
	...
	const formattedTimestamp =
		timestamp instanceof Date ? formatRelativeTime(timestamp) : timestamp
			...
			<div className="flex min-w-0 flex-1 flex-col gap-1">
				{title && (
					<div className="flex items-start justify-between gap-2">
						<p
							className={cn('text-sm', isUnread && !isCompact && 'font-medium')}
						>
							{title}
						</p>
						{isUnread && !isCompact && (
							<Badge
								variant="default"
								className="size-2 shrink-0 rounded-full p-0"
							/>
						)}
					</div>
				)}

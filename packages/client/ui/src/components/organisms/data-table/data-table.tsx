...
export function DataTable<TData>({
	...
	const tableBody = React.useMemo(() => {
		...
		const bodyRows = isDraggable
			...
			: rows.map((row) => (
					<StaticRow key={row.id} row={row} variant={variant} />
				))
		...
		const body = (
			<TableBody
				className={cn(
					...
					isContentManagerVariant && 'bg-background divide-y divide-border',
				)}
			>
				{bodyRows}
			</TableBody>
	...
	const tableContent = (
		...
		>
			<Table
				className={
					isContentManagerVariant ? 'min-w-full divide-y divide-border' : ''
				}
			>
				<TableHeader
					className={cn(
						...
						isContentManagerVariant && 'bg-muted shadow-sm',
					)}
				>

	...
	open(options: DrawerOptions = {}): DrawerInstance {
		...
		const DrawerComponent = () => {
			...
			React.useEffect(() => {
				...
				if (drawerInstance) {
					;(
						drawerInstance as unknown as {
							update: (opts: Partial<DrawerOptions>) => void
						}
				...
				>
					<DrawerContent className={currentOptions.className}>
						{(currentOptions.title || currentOptions.description) && (
							<DrawerHeader>
								{currentOptions.title && (
									<DrawerTitle>{currentOptions.title}</DrawerTitle>
								)}
						...
						{currentOptions.footer && (
							<DrawerFooter>{currentOptions.footer}</DrawerFooter>
						)}
					</DrawerContent>
				</Drawer>
			)

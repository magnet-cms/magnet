...
export async function runDbReset(options: { force?: boolean }): Promise<void> {
  ...
  if (!options.force) {
    ...
    const ok = await confirm({
      message: 'This will destroy all database data and recreate containers. Continue?',
    })
    if (!ok) {
      console.log('Aborted.')
      process.exit(0)
  ...
  try {
    ...
    console.log(ansis.yellow('⚠ Health check timed out — containers may still be starting'))
  }

...
export interface DrizzleAdapterModule {
	...
	SchemaBridge: new () => {
		/* schema bridge instance */
	}
	SchemaDiff: new (
		bridge: unknown,
	) => {
		/* schema diff instance */
	}
...
export function registerMigrateCommands(
	...
	migrate
		...
		.option(
			'--to <id>',
			'Apply migrations up to (and including) this migration ID',
		)
		...
		.action(async () => {
			try {
				...
				for (const migration of pending) {
					console.log(
						`  ${ansis.yellow('○')} ${migration.id} ${ansis.dim('(pending)')}`,
					)
				}
	...
	migrate
		...
		.action(
			async (
				...
				opts: { dryRun?: boolean; dialect?: string },
			) => {
	...
	migrate
		...
		.action(async (opts: { force?: boolean }) => {
			try {
				...
				console.log(
					ansis.green(
						`✓ Reset: rolled back ${result.rolledBack}, applied ${result.applied}`,
					),
				)
				process.exit(0)
			} catch (err) {
				console.error(
					ansis.red('✗ Reset failed:'),
					err instanceof Error ? err.message : err,
				)
				process.exit(1)
			}
	...
	migrate
		...
		.action(async (opts: { force?: boolean }) => {
			try {
				...
				console.log(
					ansis.green(
						`✓ Fresh: dropped ${result.dropped} tables, applied ${result.applied}`,
					),
				)
				process.exit(0)
			} catch (err) {
				console.error(
					ansis.red('✗ Fresh failed:'),
					err instanceof Error ? err.message : err,
				)
				process.exit(1)
			}

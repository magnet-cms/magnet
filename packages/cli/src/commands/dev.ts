...
import { findComposeFile, isDockerAvailable, runCompose, waitForHealthy } from '../utils/docker'
...
export async function runDev(): Promise<void> {
  ...
  if (!dockerAvailable) {
    ...
    try {
      ...
      console.log(ansis.yellow('⚠ Health check timed out — starting app anyway\n'))
    }
  }

...
const CONFIG_FILES = ['magnet.config.ts', 'magnet.config.js', 'magnet.config.mjs']
...
export class ConfigLoader {
  ...
  private normalizeConfig(raw: Record<string, unknown>): MagnetCliConfig {
    ...
    if (!databaseUrl || typeof databaseUrl !== 'string' || databaseUrl === 'undefined') {
      throw new Error('Config file must export a databaseUrl string or set DATABASE_URL env var.')
    }

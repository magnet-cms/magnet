...
describe('createConnection', () => {
  ...
  it('throws with helpful message listing supported schemes', async () => {
    await expect(createConnection('redis://localhost')).rejects.toThrow('postgresql://')
  })
  ...
  it('accepts postgresql:// scheme without error on scheme detection', async () => {
    ...
    const err = await createConnection('postgresql://localhost/test').catch((e) => e as Error)
    expect(err.message).not.toContain('Unsupported database URL scheme')
  })

  it('accepts postgres:// alias scheme', async () => {
    const err = await createConnection('postgres://localhost/test').catch((e) => e as Error)
    expect(err.message).not.toContain('Unsupported database URL scheme')
  })

  it('accepts mysql:// scheme without routing error', async () => {
    const err = await createConnection('mysql://localhost/test').catch((e) => e as Error)
    expect(err.message).not.toContain('Unsupported database URL scheme')
  })

  it('accepts .db SQLite file path', async () => {
    const result = await createConnection('/tmp/test.db').catch((e) => e as Error)

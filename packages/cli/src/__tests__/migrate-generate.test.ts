...
import { MigrationGenerator, SchemaBridge, SchemaDiff } from '@magnet-cms/adapter-db-drizzle'
...
describe('runMigrateGenerate', () => {
  it('returns isEmpty=true when no schema changes', async () => {
    ...
    const result = await runMigrateGenerate(diff, gen, 'add_users', '/tmp/migrations', {
      dryRun: false,
      dialect: 'postgresql',
    })
  ...
  it('includes dangerous flag in result when warnings exist', async () => {
    ...
    const result = await runMigrateGenerate(diff, gen, 'drop_users', '/tmp/migrations', {
      dryRun: true,
      dialect: 'postgresql',
    })

...
import { AdminServeModule, type AdminServeOptions } from './modules/admin-serve/admin-serve.module'
...
export function buildMagnetImports(params: BuildImportsParams): {
  ...
  const VaultModuleConfig = VaultModule.forRoot(
    ...
    categorized.vault?.adapterFactory as ((moduleRef: unknown) => VaultAdapter) | undefined,
    {
      ...(categorized.vault?.config as { cacheTtl?: number } | undefined),
      adapter: categorized.vault?.adapterType as 'db' | 'hashicorp' | 'supabase' | undefined,
    },
  )
  const AuthModuleConfig = AuthModule.forRoot(categorized.auth?.config ?? { strategy: 'jwt' })
  ...
  if (_openapi !== false && (_openapi as OpenAPIConfig | undefined)?.enabled !== false) {
    imports.push(
      OpenAPIModule.forRoot(typeof _openapi === 'object' && _openapi !== null ? _openapi : {}),
    )
  }

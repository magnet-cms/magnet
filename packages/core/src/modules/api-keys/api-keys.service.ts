...
export class ApiKeyService {
  ...
  async create(userId: string, dto: CreateApiKeyDto): Promise<CreateApiKeyResult> {
    ...
    if (existingKeys.length >= settings.maxKeysPerUser) {
      throw new Error(`Maximum API keys per user (${settings.maxKeysPerUser}) reached`)
    }
  ...
  async checkRateLimit(key: ApiKey): Promise<RateLimitResult> {
    ...
    const usageCount = usageRecords.filter((u) => u.timestamp >= windowStart).length
   ...
   */
  async getUsageHistory(keyId: string, limit = 100, offset = 0): Promise<ApiKeyUsage[]> {
  ...
  async rotate(keyId: string, userId: string): Promise<CreateApiKeyResult> {
    ...
    this.logger.log(`Rotated API key ${oldKey.keyPrefix} -> ${result.key.keyPrefix}`)
  ...
  async cleanupUsageRecords(retentionDays?: number): Promise<number> {
    ...
    cutoff.setDate(cutoff.getDate() - (retentionDays ?? settings.usageRetentionDays))

...
import { API_KEY_PERMISSION_KEY, API_KEY_SCHEMA_KEY } from '../decorators/api-key.decorator'
...
export class ApiKeyGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!this.apiKeyService.isIpAllowed(key, clientIp)) {
      this.logger.warn(`API key ${key.keyPrefix} denied: IP ${clientIp} not allowed`)
      throw new ForbiddenException('IP address not allowed')
    }

    // Check rate limit
    const rateLimit = await this.apiKeyService.checkRateLimit(key)
    this.setRateLimitHeaders(response, key.rateLimit, rateLimit.remaining, rateLimit.resetAt)
    ...
    if (requiredPermission && !this.apiKeyService.hasPermission(key, requiredPermission)) {
      this.logger.warn(`API key ${key.keyPrefix} denied: missing permission ${requiredPermission}`)
      throw new ForbiddenException(`Insufficient permissions: ${requiredPermission} required`)
    }

    // Check required schema access
    const requiredSchema = this.reflector.get<string>(API_KEY_SCHEMA_KEY, context.getHandler())
    if (requiredSchema && !this.apiKeyService.hasSchemaAccess(key, requiredSchema)) {
      this.logger.warn(`API key ${key.keyPrefix} denied: no access to schema ${requiredSchema}`)

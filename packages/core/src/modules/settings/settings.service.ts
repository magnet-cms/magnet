...
export class SettingsService implements OnApplicationBootstrap {
  ...
  async getSettings(): Promise<Record<string, Setting[]>> {
    ...
    try {
      ...
      return settings.reduce((acc: Record<string, Setting[]>, setting: Setting) => {
  ...
  async getSettingsByGroupAndKey(group: string, key: string): Promise<Setting | null> {
    const modelReady = await this.waitForModel()
    ...
    try {
      ...
      this.logger.warn(`Settings model not ready, returning null for ${resolvedGroup}/${key}`)
      return null
    }
  }
  ...
  getDefaultsByGroup(group: string): Record<string, unknown> | null {
    ...
    const settingFieldMetadata: SettingFieldMetadata[] = getSettingFields(schema)
  ...
  async getSetting(key: string): Promise<Setting | null> {
    ...
    try {
      ...
      this.logger.warn(`Settings model not ready, returning null for key: ${key}`)
      return null
    }
  }

  async updateSetting(key: string, value: SettingValue, group?: string): Promise<Setting | null> {
  ...
  async registerSetting(
    ...
    if (existingSetting) {
      ...
      if (existingSetting.type !== type) {
        return this.settingModel.update({ id: existingSetting.id } as Partial<Setting>, { type })
      }
  ...
  async registerSettings(group: string, settings: SchemaSetting[]): Promise<void> {
  ...
  async registerSettingsFromSchema<T>(group: string, schema: new () => T): Promise<void> {
    ...
    if (settingFieldMetadata.length > 0) {
      ...
      const settingsToRegister: SchemaSetting[] = settingFieldMetadata.map((field) => {
        const key = String(field.propertyKey)
    ...
    const settingsToRegister: SchemaSetting[] = fields.map(({ propertyKey, designType, type }) => {
      const key: string = propertyKey.toString()
      let value = (instance as Record<string, unknown>)[key] as SettingValue
      if (value === undefined) {
        const propData = propMetadataArray.find((p) => p.propertyKey === propertyKey)
        if (propData?.options) {
   ...
   */
  async getTypedSettings<T extends object>(group: string, schema: Type<T>): Promise<T> {
   ...
   * ```
   */
  async update<T extends object>(settingsClass: Type<T>, updates: Partial<T>): Promise<T> {
    const options = this.getSettingsOptionsOrThrow(settingsClass)
    ...
    for (const [key, value] of Object.entries(updates)) {
      ...
      const existingSetting = await this.getSettingsByGroupAndKey(options.group, settingKey)
   ...
   */
  private getSettingsOptionsOrThrow<T>(settingsClass: Type<T>): SettingsDecoratorOptions {
    const options = getSettingsOptions(settingsClass)
    if (!options) {
      throw new Error(

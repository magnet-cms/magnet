...
export class SettingsController {
  ...
  async getSetting(@Param('group') group: string, @Param('key') key: string) {
    const setting = await this.settingsService.getSettingsByGroupAndKey(group, key)
    if (!setting) {
  ...
  @Put(':group')
  async updateSettingsByGroup(@Param('group') group: string, @Body() data: SettingsRecord) {
    try {
      const results: SettingsRecord = {}

      // Update each setting in the group
      for (const [key, value] of Object.entries(data) as [string, SettingValue][]) {
        const updated = await this.settingsService.updateSetting(key, value, group)
        if (updated) {

import { ControllerMetadata, MethodMetadata, SchemaMetadata } from '@magnet-cms/common'
...
export class DiscoveryService {
  ...
  getDiscoveredSchema(name: string): SchemaMetadata | { error: string } {
    ...
    const found =
      ...
      ) || this.schemas.find((schema) => schema.className?.toLowerCase() === name.toLowerCase())
  ...
  getDiscoveredSettingsSchemas(): string[] {
    ...
    return this.settingsSchemas.map((schema) => schema.apiName || schema.name.toLowerCase())
  }
  ...
  getDiscoveredSettingsSchema(name: string): SchemaMetadata | { error: string } {
  ...
  getDiscoveredController(name: string): ControllerMetadata | { error: string } {
    return (
  ...
  getMethodDetails(path: string, methodName: string): MethodMetadata | { error: string } {
    return this.methodDiscovery.getMethodDetails(path, methodName, this.controllers)
  }
}

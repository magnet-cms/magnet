...
export class MetadataExtractorService {
  ...
  extractMethodMetadata(instance: object, method: string): MethodMetadata | null {
    ...
    const httpMethod = requestMethodMap[httpMethodNumber as RequestMethod] || 'UNKNOWN'
    ...
    if (resolveMetadata) {
      if (resolveMetadata.type) {
        const isArray =
          ...
          (typeof resolveMetadata.type === 'function' && resolveMetadata.isArray)
  ...
  extractSchemaMetadata(wrapper: InstanceWrapper<unknown>): SchemaMetadata | null {
    ...
    const fieldMetadata: unknown[] = Reflect.getMetadata(FIELD_METADATA_KEY, metatype) ?? []
    ...
    const settingFieldPropertyKeys = new Set(
      settingFieldMetadata.filter(isSettingFieldMetadata).map((f) => String(f.propertyKey)),
    )
    ...
    const kebabName = metatype.name.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase()
    ...
    const settingsOptions: SettingsDecoratorOptions | undefined = Reflect.getMetadata(
      SETTINGS_OPTIONS_METADATA_KEY,
      metatype,
    )
  ...
  private buildProperties(
    ...
    for (const meta of fieldMetadata) {
      ...
      const designTypeMetadata = Reflect.getMetadata(DESIGN_TYPE, metatype.prototype, propertyKey)
    ...
    for (const meta of settingFieldMetadata) {
      ...
      const isMultiSelect =
        meta.type === 'select' && 'multiple' in meta.options && Boolean(meta.options.multiple)

      properties.push({
        name: propertyKey,
        type: this.getTypeNameFromSettingFieldMetadata(meta, designTypeMetadata),
    ...
    for (const prop of schemaProps) {
      ...
      const uiField = uiMetadata.find((ui) => ui.propertyKey === prop.propertyKey)
   ...
   */
  private getTypeNameFromFieldMetadata(meta: FieldMetadata, designType: unknown): string {
  ...
  private getTypeName(type: unknown, designType: unknown): string {
    ...
    if (designType && typeof designType === 'function' && 'name' in designType) {
  ...
  private isArrayField(meta: FieldMetadata): boolean {
    ...
    if (meta.type === 'relationship' && meta.options && 'multiple' in meta.options) {
   ...
   */
  private buildUIFromFieldMetadata(meta: FieldMetadata): UIFieldMetadata['options'] | undefined {
    ...
    const uiTypeMap: Record<string, string> = {
      ...
      number: 'number',
      boolean:
        options && 'style' in options && options.style === 'checkbox' ? 'checkbox' : 'switch',
      date: 'date',
      datetime: 'date',
      richtext: 'richText',
      markdown: 'textarea',
    ...
    if ((type === 'select' || type === 'enum') && 'options' in options) {
      uiOptions.options = this.convertSelectOptions(options as Record<string, unknown>)
    }
  ...
  private buildUIFromSettingFieldMetadata(
    ...
    if (type === 'select' && 'options' in options) {
      uiOptions.options = this.convertSettingSelectOptions(options as Record<string, unknown>)
      // Handle multi-select
  ...
  getParamDetails(controller: Function, methodName: string) {
    ...
    const paramTypes: Type<unknown>[] =
      Reflect.getMetadata(PARAMTYPES_METADATA, controller.prototype, methodName) || []

    const paramDetails: Array<{ arg: string; type: string; name: string }> = []

    for (const [index, [key, metadata]] of Object.entries(routeArgsMetadata).entries()) {

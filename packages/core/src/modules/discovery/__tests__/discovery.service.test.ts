...
const makeSchema = (name: string, opts?: SchemaMetadata['options']): SchemaMetadata => ({
  name,
  apiName: name,
...
describe('DiscoveryService schema filtering', () => {
  ...
  it('should filter out excluded schema names (setting, history)', () => {
    const schemas = [makeSchema('article'), makeSchema('setting'), makeSchema('history')]
    const result = filterSchemas(schemas)
  ...
  it('should filter out schemas with visible: false', () => {
    const schemas = [makeSchema('article'), makeSchema('internal', { visible: false })]
    const result = filterSchemas(schemas)
    expect(result).toHaveLength(1)
    expect(result[0].name).toBe('article')
  })
  ...
  it('should keep read-only schemas (readOnly does not hide from list)', () => {
    const schemas = [makeSchema('article'), makeSchema('audit-log', { readOnly: true })]
    const result = filterSchemas(schemas)
    expect(result).toHaveLength(2)
  })

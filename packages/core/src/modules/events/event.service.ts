...
export class EventService implements OnModuleDestroy {
   ...
   */
  getHandlers(): Map<string, Array<{ name: string; priority: number; async: boolean }>> {
    const result = new Map<string, Array<{ name: string; priority: number; async: boolean }>>()
  ...
  private logHandlerError(
    ...
    this.logger.error(`Handler '${options.name}' for event '${event}' failed: ${message}`, stack)
  }

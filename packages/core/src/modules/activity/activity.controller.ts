...
export class ActivityController {
  ...
  async getRecent(@Query('limit') limit?: string): Promise<Activity[]> {
    ...
    return this.activityService.getRecent(Number.isNaN(parsedLimit) ? 50 : parsedLimit)
  }
  ...
  async getByUser(
    ...
    const parsedLimit = limit ? Number.parseInt(limit, 10) : 100
    return this.activityService.getByUser(userId, Number.isNaN(parsedLimit) ? 100 : parsedLimit)
  }
  ...
  async search(
    ...
    if (action !== undefined && !VALID_EVENT_NAMES.has(action)) {
      throw new BadRequestException(`Invalid action: "${action}". Must be a known event name.`)
    }
   ...
   */
  @Delete('cleanup')
  async cleanup(@Query('retentionDays') retentionDays?: string): Promise<{ deleted: number }> {
    const days = retentionDays ? Number.parseInt(retentionDays, 10) : 90
    const deleted = await this.activityService.cleanup(Number.isNaN(days) ? 90 : days)
    return { deleted }
  }
}

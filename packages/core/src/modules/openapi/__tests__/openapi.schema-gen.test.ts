...
describe('mapFieldTypeToOASSchema', () => {
	it('maps text → string', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'text' })),
		).toEqual({ type: 'string' })
	})

	it('maps number → number', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'number' })),
		).toEqual({ type: 'number' })
	})

	it('maps boolean → boolean', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'boolean' })),
		).toEqual({ type: 'boolean' })
	})

	it('maps date → string with date-time format', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'date' })),
		).toEqual({
			type: 'string',
			format: 'date-time',
		})
	...
	it('maps datetime → string with date-time format', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'datetime' })),
		).toEqual({
			type: 'string',
			format: 'date-time',
		})
	...
	it('maps json → object', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'json' })),
		).toEqual({ type: 'object' })
	})

	it('maps object → object', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'object' })),
		).toEqual({ type: 'object' })
	})

	it('maps richtext → string', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'richtext' })),
		).toEqual({ type: 'string' })
	})

	it('maps markdown → string', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'markdown' })),
		).toEqual({ type: 'string' })
	})

	it('maps image → string', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'image' })),
		).toMatchObject({ type: 'string' })
	})

	it('maps file → string', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'file' })),
		).toMatchObject({ type: 'string' })
	})

	it('maps relationship → string', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'relationship' })),
		).toMatchObject({ type: 'string' })
	})

	it('maps tags → array of strings', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'tags' })),
		).toEqual({
			type: 'array',
			items: { type: 'string' },
		})
	...
	it('maps array → array of strings', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'array' })),
		).toEqual({
			type: 'array',
			items: { type: 'string' },
		})
	...
	it('maps blocks → array of objects', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'blocks' })),
		).toEqual({
			type: 'array',
			items: { type: 'object' },
		})
	...
	it('maps select → string', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'select' })),
		).toMatchObject({ type: 'string' })
	})

	it('maps enum → string', () => {
		expect(
			mapFieldTypeToOASSchema(makeProp({ name: 'x', type: 'enum' })),
		).toMatchObject({ type: 'string' })
	})
	...
	it('maps unknown type → string (fallback)', () => {
		expect(
			mapFieldTypeToOASSchema(
				makeProp({ name: 'x', type: 'unknown-future-type' }),
			),
		).toEqual({ type: 'string' })
	})
})
...
describe('enrichDocumentWithSchemas', () => {
	...
	it('should wrap array properties with isArray=true', () => {
		const doc = makeDoc()
		const schema = makeSchema('article', [
			makeProp({ name: 'tags', type: 'text', isArray: true }),
		])
		enrichDocumentWithSchemas(doc, [schema])

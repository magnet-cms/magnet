...
export class WebhookListenerService implements OnModuleInit {
	...
	private async cleanupOldDeliveries(): Promise<void> {
		try {
			...
			this.logger.log(
				`Cleaned up delivery logs older than ${retentionDays} days`,
			)
		} catch (err) {
			this.logger.warn(
	...
	private async handleEvent(
		...
		try {
			...
			const outgoingPayload = await this.webhookService.enrichPayload(
				eventName,
				enrichedData,
			)
			...
			const deliveries = webhooks.map(async (webhook) => {
				try {
					const fullWebhook = await this.webhookService.findByIdWithSecret(
						webhook.id,
					)
					if (!fullWebhook) return

					await this.webhookService.deliverWithRetry(
						fullWebhook,
						eventName,
						outgoingPayload,
					)

...
describe('WebhookService', () => {
  ...
  describe('create', () => {
    ...
    it('should use provided secret instead of generating one', async () => {
      mockWebhookModel.create.mockImplementation((data) => Promise.resolve({ ...data, id: 'wh-2' }))
    ...
    it('should default enabled to true', async () => {
      mockWebhookModel.create.mockImplementation((data) => Promise.resolve({ ...data, id: 'wh-3' }))
  ...
  describe('deliver', () => {
    ...
    it('should include correct signature header', async () => {
      ...
      const expectedSig = service.sign(JSON.stringify(payload), sampleWebhook.secret)
  ...
  describe('deliverWithRetry', () => {
    ...
    it('should succeed on first attempt and not retry', async () => {
      ...
      const resultPromise = service.deliverWithRetry(sampleWebhook, 'content.created', { id: '1' })
    ...
    it('should log each delivery attempt', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ status: 500, text: async () => 'Error' }))
      mockSettingsService.get.mockResolvedValue({
        maxRetries: 1,
        timeoutMs: 5000,
      })

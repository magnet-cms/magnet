...
export class ViewConfigService {
   ...
   */
  async get(userId: string, schemaName: string): Promise<BaseSchema<ViewConfig> | null> {
  ...
  async upsert(
    ...
    if (existing) {
      return this.viewConfigModel.update({ id: existing.id } as Partial<BaseSchema<ViewConfig>>, {

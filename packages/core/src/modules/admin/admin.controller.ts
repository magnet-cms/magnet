...
export class AdminController {
  ...
  @All('admin/*path')
  async handleAllRequests(@Req() req: Request, @Res() res: Response, @Next() next: NextFunction) {
    return this.serveAdmin(req, res, next)
  }

  @Get('admin')
  async getAdminRoot(@Req() req: Request, @Res() res: Response, @Next() next: NextFunction) {
    return this.serveAdmin(req, res, next)
  }
  ...
  private async serveAdmin(req: Request, res: Response, next: NextFunction) {
    ...
    if (this.adminService.isDevMode()) {
      try {
        ...
        return res
          .status(502)
          .send('Admin panel is not available. Make sure the admin client is running on port 3001.')
      }
      return
    }
    ...
    if (!this.adminService.hasAdminFiles()) {
      return res
        .status(404)
        .send('Admin panel not found. Make sure @magnet-cms/admin is installed and built.')
    }

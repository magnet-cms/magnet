...
export class AdminService {
  ...
  constructor(
    ...
    this.adminDistPath = process.env.MAGNET_ADMIN_PATH || this.findAdminDistPath()
  ...
  private findAdminDistPath(): string {
    ...
    const monorepoPath = findPathInParentDirectories('packages/client/admin/dist/client')
    if (monorepoPath) {
      return monorepoPath
    }

    // Installed as dependency (fallback)
    const nodeModulesPath = resolve(process.cwd(), 'node_modules/@magnet-cms/admin/dist/client')
  ...
  async getInitialConfig(): Promise<InitialConfig> {
    ...
    const titleValue = overview.find((setting) => setting.key === 'title')?.value
    const descriptionValue = overview.find((setting) => setting.key === 'description')?.value

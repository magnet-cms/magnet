...
import { PasswordResetRequestResult, PasswordResetService } from './services/password-reset.service'
...
export class AuthService {
  ...
  async login(
    ...
    if (settings.enableSessions && settings.maxConcurrentSessions > 0) {
      await this.enforceConcurrentSessionLimit(effectiveUserId, settings.maxConcurrentSessions)
    }
  ...
  async logout(refreshToken: string): Promise<void> {
    ...
    if (storedToken) {
      ...
      await this.sessionModel.update({ refreshTokenId: tokenHash }, { active: false })
  ...
  async logoutAll(userId: string): Promise<void> {
    ...
    for (const session of sessions) {
      await this.sessionModel.update({ sessionId: session.sessionId }, { active: false })
    }
   ...
   * Validate password against policy
   */
  async validatePasswordPolicy(password: string): Promise<PasswordValidationResult> {
    const settings = await this.settingsService.get(AuthSettings)
    const errors: string[] = []
  ...
  async changePassword(
    ...
    const validation = await this.validatePasswordPolicy(changePasswordDto.newPassword)
    if (!validation.valid) {
      throw new InvalidCredentialsError(validation.errors.join(', '))
    }
  ...
  async updateProfile(
    ...
    if (updateProfileDto.email && updateProfileDto.email !== user.email) {
      ...
      if (existingUser) throw new DuplicateKeyError('email', updateProfileDto.email)
    }
   ...
   */
  async loginWithOAuthProfile(user: AuthUser, context: RequestContext = {}): Promise<AuthResult> {
    const settings = await this.settingsService.get(AuthSettings)

    // Enforce concurrent session limit if configured
    if (settings.enableSessions && settings.maxConcurrentSessions > 0) {
      await this.enforceConcurrentSessionLimit(user.id, settings.maxConcurrentSessions)
    }
    ...
    let sessionId: string | undefined
    if (settings.enableSessions) {
      const session = await this.createSession(user.id, refreshTokenData.id, context, settings)
      sessionId = session.sessionId
    }
  ...
  private async isAccountLocked(email: string, settings: AuthSettings): Promise<boolean> {
    ...
    const recentFailures = attempts.filter((a) => !a.success && a.timestamp >= windowStart)
   ...
   */
  private async revokeRefreshToken(tokenHash: string, reason: string): Promise<void> {
    await this.refreshTokenModel.update(
      { token: tokenHash },
      {
        revoked: true,
  ...
  private async emitEvent(event: string, payload: Record<string, unknown>): Promise<void> {
    try {
      ...
      await this.eventService.emit(event as Parameters<typeof this.eventService.emit>[0], payload)
    } catch (error) {

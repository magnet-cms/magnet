...
export class PermissionDiscoveryService implements OnModuleInit {
  ...
  private async discoverPermissions(): Promise<void> {
    ...
    await this.permissionService.upsertMany(Array.from(this.permissions.values()))
  ...
  private extractControllerPermissions(wrapper: InstanceWrapper): void {
    ...
    for (const methodName of Object.getOwnPropertyNames(prototype)) {
      ...
      const permOptions = Reflect.getMetadata(PERMISSION_METADATA_KEY, method) as
        | PermissionOptions
        | undefined
  ...
  private groupByController(perms: PermissionDefinition[]): PermissionGroup[] {
    ...
    return Array.from(byController.entries()).map(([controllerName, controllerPerms]) => {
  ...
  getSchemaPermissions(schemaName: string): PermissionDefinition[] {
    return this.getAll().filter(
      (p) =>
        p.schema?.toLowerCase() === schemaName.toLowerCase() || p.apiId === `api::${schemaName}`,
    )
  }

...
export class RoleService implements OnModuleInit {
  ...
  private getCachedPermission(roleName: string, permission: string): boolean | undefined {
    const roleCache = this.permissionCache.get(roleName)
    return roleCache?.get(permission)
  }

  private setCachedPermission(roleName: string, permission: string, value: boolean): void {
  ...
  private resolveRolePermissions(role: Role): RoleWithPermissions {
    ...
    return {
      ...
      plugins: this.permissionDiscovery.markPermissions(categorized.plugins, role.permissions),
      system: this.permissionDiscovery.markPermissions(categorized.system, role.permissions),
    }
  }

...
describe('RoleService', () => {
  ...
  describe('checkPermission', () => {
    ...
    it('should allow exact permission match', () => {
      expect(service.checkPermission(['content.create'], 'content.create')).toBe(true)
    })

    it('should allow partial wildcard (content.*)', () => {
      expect(service.checkPermission(['content.*'], 'content.posts.create')).toBe(true)
    })

    it('should allow nested partial wildcard (content.posts.*)', () => {
      expect(service.checkPermission(['content.posts.*'], 'content.posts.create')).toBe(true)
    })

    it('should deny permission not in list', () => {
      expect(service.checkPermission(['content.read'], 'content.delete')).toBe(false)
    })
    ...
    it('should not match parent namespace as wildcard', () => {
      ...
      expect(service.checkPermission(['content'], 'content.posts.create')).toBe(false)
    })
  })

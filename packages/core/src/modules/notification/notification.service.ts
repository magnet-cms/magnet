...
export class NotificationService implements OnModuleInit {
	...
	registerAdapter(adapter: NotificationChannelAdapter): void {
		const existing = this.adapters.findIndex(
			(a) => a.channel === adapter.channel,
		)
		if (existing !== -1) {
		...
		this.logger.log(
			`Registered notification adapter for channel '${adapter.channel}'`,
		)
	}
	...
	async notify(dto: NotifyDto): Promise<void> {
		try {
			...
			await Promise.all(
				userIds.map((userId) =>
					this.deliverToUser({ ...dto, userId, channels }, settings),
				),
			)
		} catch (error) {
			this.logger.warn(
	...
	async cleanup(retentionDays?: number): Promise<number> {
		...
		const results = await Promise.all(
			old.map((n) =>
				this.notificationModel.delete({ id: n.id } as Partial<Notification>),
			),
		)

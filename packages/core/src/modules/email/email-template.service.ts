...
export class EmailTemplateService implements OnModuleInit {
   ...
   */
  async findAll(filters?: EmailTemplateFilters): Promise<BaseSchema<EmailTemplate>[]> {
   ...
   */
  async findBySlug(slug: string, locale?: string): Promise<BaseSchema<EmailTemplate> | null> {
    const all = await this.templateModel.findMany({ slug } as Partial<BaseSchema<EmailTemplate>>)
   ...
   */
  async findBySlugAllLocales(slug: string): Promise<BaseSchema<EmailTemplate>[]> {
    const all = await this.templateModel.findMany({ slug } as Partial<BaseSchema<EmailTemplate>>)
    return all ?? []
  }
   ...
   */
  async create(data: CreateEmailTemplateData): Promise<BaseSchema<EmailTemplate>> {
  ...
  async delete(id: string): Promise<boolean> {
    return this.templateModel.delete({ id } as Partial<BaseSchema<EmailTemplate>>)
  }
  ...
  private applyFilters(
    ...
    if (filters.search) {
      ...
      result = result.filter(
        (t) => t.slug.toLowerCase().includes(search) || t.subject.toLowerCase().includes(search),
      )
    }

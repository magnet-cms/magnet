 ...
 */
function applyFilters(templates: FakeTemplate[], filters?: EmailTemplateFilters): FakeTemplate[] {
 ...
 */
function findBySlugAllLocales(templates: FakeTemplate[], slug: string): FakeTemplate[] {
 ...
 */
function isSlugLocaleTaken(existing: FakeTemplate[], slug: string, locale: string): boolean {
...
function makeVersion(overrides: Partial<EmailTemplateVersion> = {}): EmailTemplateVersion {
  return {
    subject: 'Old Subject',
    body: '<p>Old</p>',
    editedBy: 'user-1',
...
describe('EmailTemplateService — version capping', () => {
  ...
  it('should keep up to MAX_VERSIONS (5) entries', () => {
    const existing = [1, 2, 3, 4, 5].map((n) => makeVersion({ subject: `v${n}` }))

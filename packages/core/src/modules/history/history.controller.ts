...
export class HistoryController {
  ...
  private mapVersion(version: History): History & { collection: string; version: number } {
    return {
      ...version,
  ...
  async getVersions(
    ...
    const versions = await this.historyService.findVersions(documentId, collection)
  ...
  async publishVersion(@Param('versionId') versionId: string): Promise<History | null> {
    return this.historyService.publishVersion(versionId)
  }

  @Put('version/:versionId/archive')
  async archiveVersion(@Param('versionId') versionId: string): Promise<History | null> {

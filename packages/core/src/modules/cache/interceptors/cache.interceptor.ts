...
export class CacheInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    const metadata = Reflect.getMetadata(CACHE_METADATA_KEY, handler) as CacheMetadata | undefined

    if (!metadata) {
      return next.handle()
    }

    const ttl = metadata.ttl ?? (Reflect.getMetadata(CACHE_TTL_KEY, handler) as number | undefined)

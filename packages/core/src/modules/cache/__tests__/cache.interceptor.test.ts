...
const mockHealthCheck = vi.fn(async (): Promise<CacheHealthResult> => ({ healthy: true }))
...
function makeContext(metadata: Record<string, unknown> | null, ttl?: number): ExecutionContext {
  return {
    getHandler: () => {
      const fn = () => {}
      if (metadata) {

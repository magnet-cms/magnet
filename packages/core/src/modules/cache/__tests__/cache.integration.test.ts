...
import { CACHE_ADAPTER_TOKEN, CACHE_METADATA_KEY, CACHE_TTL_KEY } from '../cache.constants'
...
describe('Cache system integration', () => {
  ...
  describe('Decorator metadata integration', () => {
    ...
    it('should support @CacheEvict with allEntries for glob patterns', () => {
      ...
      const meta = Reflect.getMetadata(CACHE_METADATA_KEY, Controller.prototype.mutate)
      expect(meta).toEqual({
        operation: 'evict',
        key: 'posts:*',
        allEntries: true,
    ...
    it('should support @CachePut for forced refresh', () => {
      ...
      const meta = Reflect.getMetadata(CACHE_METADATA_KEY, Controller.prototype.refresh)

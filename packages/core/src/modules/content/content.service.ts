...
export class ContentService {
  ...
  private getModel<T>(schemaName: string): Model<T> {
    ...
    for (const token of tokens) {
      ...
      try {
        ...
        if (model && typeof (model as unknown as Record<string, unknown>).findMany === 'function') {
          return model
        }
      } catch {
  ...
  async list<T>(schemaName: string, options: ListDocumentOptions = {}) {
    ...
    if (schemaName.toLowerCase() === 'user' && this.authStrategy?.name === 'supabase') {
      ...
      if (typeof strategyWithListUsers.listUsers === 'function') {
        try {
          ...
          this.logger.warn('Failed to list users from Supabase Auth, falling back to database')
        }
      }
    }
   ...
   */
  async findPublished<T>(schemaName: string, documentId: string, locale?: string) {
    const model = this.getModel<T>(schemaName)
   ...
   */
  async create<T>(schemaName: string, data: Partial<T>, options: CreateDocumentOptions = {}) {
    const model = this.getModel<T>(schemaName)
    ...
    const result = await this.documentService.create(model, sanitizedData, options)
  ...
  async update<T>(
    ...
    const result = await this.documentService.update(model, documentId, sanitizedData, options)
   ...
   * Publish a document
   */
  async publish<T>(schemaName: string, documentId: string, options: PublishDocumentOptions = {}) {
    const model = this.getModel<T>(schemaName)
    const result = await this.documentService.publish(model, documentId, schemaName, options)
  ...
  async unpublish<T>(schemaName: string, documentId: string, locale?: string) {
    const model = this.getModel<T>(schemaName)
    const result = await this.documentService.unpublish(model, documentId, locale)
   ...
   */
  async deleteLocale<T>(schemaName: string, documentId: string, locale: string) {
    const model = this.getModel<T>(schemaName)
  ...
  async addLocale<T>(
    ...
    return this.documentService.addLocale(model, documentId, locale, sanitizedData, options)
  }
  ...
  async getVersions(schemaName: string, documentId: string, locale?: string) {
    if (locale) {
      return this.historyService.findVersionsByLocale(documentId, schemaName, locale)
    }

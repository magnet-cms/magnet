...
export class DbVaultAdapter extends VaultAdapter {
  ...
  async set(key: string, value: string, description?: string): Promise<void> {
    if (!this.configured) {
      throw new Error('Vault is not configured. Set VAULT_MASTER_KEY to enable secret storage.')
    }
  ...
  async list(prefix?: string): Promise<VaultSecretMeta[]> {
    ...
    if (prefix) {
      ...
      const results = await builder
        .where({ key: { $regex: `^${prefix}` } } as Parameters<typeof builder.where>[0])
        .exec()
  ...
  private getModel(): Model<VaultSecret> {
    if (!this.model) {
      throw new Error('DbVaultAdapter not initialized. Call initialize() first.')
    }
    return this.model
  }
  ...
  private decrypt(encryptedData: string, iv: string, authTag: string): string {
    const decipher = createDecipheriv(ALGORITHM, this.getMasterKey(), Buffer.from(iv, 'hex'), {
      authTagLength: AUTH_TAG_LENGTH,
    })

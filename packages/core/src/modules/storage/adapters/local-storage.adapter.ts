...
export class LocalStorageAdapter implements StorageAdapter {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.logger.warn('Sharp not available - image transforms will be disabled')
    }
  ...
  async upload(
    ...
    const targetDir = folder ? path.join(this.config.uploadDir, folder) : this.config.uploadDir
    ...
    if (buffer.length > this.config.maxFileSize) {
      throw new Error(`File size exceeds limit of ${this.formatSize(this.config.maxFileSize)}`)
    }
  ...
  async transform(storagePath: string, options: TransformOptions): Promise<Buffer> {
    if (!this.sharp) {
      throw new Error('Image transforms are not available (sharp not installed)')
    }

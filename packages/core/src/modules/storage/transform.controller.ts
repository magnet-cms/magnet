...
export class TransformController {
  ...
  async getFile(
    ...
    if (!res) {
      throw new HttpException('Response object required', HttpStatus.INTERNAL_SERVER_ERROR)
    }
    ...
    try {
      ...
      const isImage = media.mimeType.startsWith('image/') && media.mimeType !== 'image/svg+xml'
      ...
      if (hasTransform && isImage) {
        ...
        if (
          ...
          throw new HttpException('Invalid width (must be 1-4000)', HttpStatus.BAD_REQUEST)
        }
        if (
          transformOptions.height &&
          (transformOptions.height < 1 || transformOptions.height > 4000)
        ) {
          throw new HttpException('Invalid height (must be 1-4000)', HttpStatus.BAD_REQUEST)
        }
        if (
          transformOptions.quality &&
          (transformOptions.quality < 1 || transformOptions.quality > 100)
        ) {
          throw new HttpException('Invalid quality (must be 1-100)', HttpStatus.BAD_REQUEST)
        }

...
interface EncryptionService {
  ...
  decrypt(buffer: Buffer, userId: string, iv: string, authTag: string): Promise<Buffer>
}
 ...
 */
async function prepareUploadBuffer(input: EncryptedUploadInput): Promise<EncryptedUploadOutput> {
  if (!input.encrypt) {
  ...
  if (!input.ownerId) {
    throw new InternalServerErrorException('ownerId is required when encrypt:true')
  }
...
const mockDecrypt = vi.fn(
  async (_buf: Buffer, _userId: string, _iv: string, _authTag: string): Promise<Buffer> =>
    Buffer.from('plaintext-bytes'),
)
...
describe('StorageService encryption orchestration', () => {
  ...
  describe('decrypt orchestration logic', () => {
    ...
    it('should not call decrypt for non-encrypted media', async () => {
      ...
      const result = isEncrypted ? await mockDecrypt(rawBuffer, 'user-1', 'iv', 'tag') : rawBuffer

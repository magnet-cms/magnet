import type { PluginConfig, PluginMetadata, PluginModuleOptions } from '@magnet-cms/common'
import type { Type } from '@nestjs/common'
import { DynamicModule, Module, Provider } from '@nestjs/common'
import { DiscoveryModule } from '@nestjs/core'

import { PLUGIN_METADATA, PLUGIN_MODULE, getPluginOptionsToken } from './constants'
...
export class PluginModule {
   ...
   * ```
   */
  static forRoot(options: PluginModuleOptions = { plugins: [] }): DynamicModule {
    ...
    return {
      ...
      exports: [PluginService, PluginRegistryService, PluginLifecycleService, ...pluginProviders],
    }
  }
   ...
   */
  static forRootLegacy(options: PluginOptions = { plugins: [] }): DynamicModule {
   ...
   */
  private static collectPluginModules(plugins: PluginConfig[]): Type<unknown>[] {
    ...
    for (const config of plugins) {
      ...
      if (pluginModule) {
        ...
        const isGetter = typeof pluginModule === 'function' && !pluginModule.prototype
  ...
  private static createPluginProviders(plugins: PluginConfig[]): Provider[] {
    ...
    for (const config of plugins) {
      const PluginClass = config.plugin
      const metadata = Reflect.getMetadata(PLUGIN_METADATA, PluginClass) as PluginMetadata

      if (!metadata) {
        throw new Error(`Plugin ${PluginClass.name} is missing @Plugin() decorator`)
      }

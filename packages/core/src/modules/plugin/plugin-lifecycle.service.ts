import type { PluginConfig, PluginLifecycle, PluginMetadata } from '@magnet-cms/common'
 ...
 */
@Injectable()
export class PluginLifecycleService implements OnModuleInit, OnApplicationShutdown {
    ...
    for (const config of this.pluginsConfig) {
      ...
      for (const module of this.modulesContainer.values()) {
        ...
        if (provider?.instance) {
          this.pluginInstances.set(metadata.name, provider.instance as PluginLifecycle)
          break
        }
      }
    }
    ...
    for (const [name, instance] of this.pluginInstances) {
      if (typeof instance.onPluginInit === 'function') {
        try {
          ...
          this.logger.error(`Error in onPluginInit for plugin "${name}":`, error)
        }
      }
    }
  }
    ...
    for (const [name, instance] of this.pluginInstances) {
      if (typeof instance.onPluginDestroy === 'function') {
        try {
          ...
          this.logger.error(`Error in onPluginDestroy for plugin "${name}":`, error)
        }
      }
    }
  }

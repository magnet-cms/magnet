...
export function validateEnvironment(
  ...
  const missingByType = new Map<string, { name: string; description?: string }[]>()
  ...
  const jwtSecretProvided = globalOptions?.jwt?.secret && globalOptions.jwt.secret.trim() !== ''
  const jwtSecretInEnv = process.env.JWT_SECRET && process.env.JWT_SECRET.trim() !== ''
  ...
  lines.push('Set these in your .env file or environment before starting the application.')

...
describe('validateEnvironment', () => {
  ...
  it('should not throw when all required env vars are present', () => {
    ...
    expect(() => validateEnvironment(providers, undefined, { exitOnFailure: false })).not.toThrow()
  })
  ...
  it('should throw when required env vars are missing', () => {
    ...
    expect(() => validateEnvironment(providers, undefined, { exitOnFailure: false })).toThrow(
    ...
    const providers: MagnetProvider[] = [
      {
        ...
        adapter: {} as DatabaseMagnetProvider['adapter'],
        config: { connectionString: '', dialect: 'postgresql' },
        envVars: [{ name: 'DATABASE_URL', required: true, description: 'DB URL' }],
      },
    ]
  ...
  it('should check JWT_SECRET from global options', () => {
    ...
    expect(() => validateEnvironment([], undefined, { exitOnFailure: false })).toThrow('JWT_SECRET')
  })

  it('should not require JWT_SECRET when provided in global options', () => {
    process.env.JWT_SECRET = 'test-secret'

    expect(() => validateEnvironment([], undefined, { exitOnFailure: false })).not.toThrow()
  })

...
function applyOpenAPIConditional(
  ...
  if (openapi !== false && (openapi as OpenAPIConfig | undefined)?.enabled !== false) {
    imports.push(
      OpenAPIModule.forRoot(typeof openapi === 'object' && openapi !== null ? openapi : {}),
    )
  }
}

function hasOpenAPI(imports: unknown[]) {
  return imports.some((m) => (m as { module?: unknown }).module === OpenAPIModule)
}
...
describe('OpenAPI conditional import in buildMagnetImports', () => {
  ...
  it('should pass config to OpenAPIModule.forRoot when object config provided', () => {
    ...
    const _mod = imports[0] as { config?: OpenAPIConfig }
    ...
    const configProvider = providers?.find((p) => p.provide === 'OPENAPI_CONFIG')

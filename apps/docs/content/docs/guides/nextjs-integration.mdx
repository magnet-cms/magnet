...
Enable admin serving in your Magnet backend from `AppModule` (or equivalent root module):
...
Use `DrizzleDatabaseAdapter.forRoot({ ... })` instead of Mongoose if you use PostgreSQL/MySQL/SQLite via Drizzle.
...
- [Configure Authentication](/docs/guides/setup-auth)
- [Create Custom Schemas](/docs/reference/common/decorators)

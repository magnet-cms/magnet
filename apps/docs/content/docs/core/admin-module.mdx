...
```typescript
// Admin UI is enabled by default — omit the option entirely
...
When `admin` is omitted, Nest serves the built admin SPA at `/admin` when a dist is found (see below). When `admin: false`, only REST APIs are available from the same app.

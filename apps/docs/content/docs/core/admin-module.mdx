...
## Enabling and Disabling Admin

Control admin UI serving via the second argument to `MagnetModule.forRoot()`:

```typescript
// Admin UI is enabled by default — omit `admin` or pass true
MagnetModule.forRoot([DrizzleDatabaseAdapter.forRoot(), ...])

MagnetModule.forRoot([...], { admin: true })

// API-only / headless mode — no admin UI served
MagnetModule.forRoot([...], { admin: false })
```

When `admin` is omitted or `true`, Nest serves the built admin SPA at `/admin` when a dist is found (see below). When `admin: false`, only REST APIs are available from the same app.

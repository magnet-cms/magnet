...
Control admin UI serving via `MagnetModule.forRoot()`:

```typescript
// Enable admin UI (default when admin: true)
MagnetModule.forRoot({ db: {...}, jwt: {...}, admin: true })

// API-only / headless mode — no admin UI served
MagnetModule.forRoot({ db: {...}, jwt: {...}, admin: false })
```

When `admin: false`, the CMS runs in headless mode: REST APIs are available but no admin static files are served.

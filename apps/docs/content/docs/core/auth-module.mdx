...
JWT is the default auth strategy. `JWT_SECRET` is auto-resolved from `process.env`.
...
`SupabaseAuthAdapter.forRoot()` auto-resolves `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`, and `SUPABASE_SECRET_KEY` from `process.env`.
...
Register your custom strategy using the `AuthStrategyFactory`:

...
const createTestFile = (): Buffer => Buffer.from('PII document content — encrypted at rest')
...
test.describe('Media Encryption & Access Control', () => {
  ...
  test('non-encrypted upload works as before (backward compat)', async ({ apiClient }) => {
  ...
  test('encrypted upload with vault returns isEncrypted:true', async ({ apiClient }) => {
    const response = await apiClient.uploadMedia(
      createTestFile(),
  ...
  test('encrypted upload auto-creates private/{userId} folder', async ({ apiClient }) => {
    const response = await apiClient.uploadMedia(
      createTestFile(),
  ...
  test('owner can access their encrypted media metadata', async ({ apiClient }) => {
    // Upload as owner
  ...
  test('non-owner gets 403 on encrypted media metadata', async ({ apiClient }) => {
    // Upload as owner
    const uploadResponse = await apiClient.uploadMedia(
      createTestFile(),
  ...
  test('unauthenticated request gets 403 on encrypted media', async ({ apiClient }) => {
  ...
  test('admin can access encrypted media owned by another user', async ({ apiClient }) => {
    // Upload as owner
    const uploadResponse = await apiClient.uploadMedia(
      createTestFile(),
  ...
  test('non-encrypted media remains accessible without auth', async ({ apiClient }) => {

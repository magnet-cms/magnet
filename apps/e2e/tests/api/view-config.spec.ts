...
test.describe('View Config API', () => {
	test.describe('GET /user-preferences/view-config/:schema', () => {
		...
		test('requires authentication', async ({ request, apiBaseURL }) => {
			const response = await request.get(
				`${apiBaseURL}/user-preferences/view-config/blog`,
			)
	...
	test.describe('PUT /user-preferences/view-config/:schema', () => {
		test('saves and returns view config', async ({
			authenticatedApiClient,
		}) => {
			const config = {
				columns: [
			...
			const response = await authenticatedApiClient.putViewConfig(
				'blog',
				config,
			)
			expect(response.ok()).toBeTruthy()
		...
		test('GET returns previously saved config', async ({
			authenticatedApiClient,
		}) => {
			const config = {

...
test.describe('Content API', () => {
  test.describe('CRUD operations', () => {
    test('GET /content/:schema responds within 2000ms', async ({ authenticatedApiClient }) => {
    ...
    test('POST /content/:schema creates a document', async ({
      ...
      const response = await authenticatedApiClient.createContent(SCHEMA, catData(ownerId))
      expect(response.ok()).toBeTruthy()
    ...
    test('GET /content/:schema lists documents', async ({ authenticatedApiClient, cleanup }) => {
      const ownerData = testData.owner.create()
      const ownerRes = await authenticatedApiClient.createOwner(ownerData)
      const owner = await ownerRes.json()
      const ownerId = (owner.id ?? owner._id) as string
      ...
      const createRes = await authenticatedApiClient.createContent(SCHEMA, catData(ownerId))
    ...
    test('GET /content/:schema/:documentId returns a document', async ({
      ...
      const response = await authenticatedApiClient.getContent(SCHEMA, created.documentId)
      expect(response.ok()).toBeTruthy()
    ...
    test('GET /content/:schema/:documentId returns 404 for nonexistent', async ({
      ...
      const response = await authenticatedApiClient.getContent(SCHEMA, 'nonexistent-doc-id')
    ...
    test('PUT /content/:schema/:documentId updates a document', async ({
      ...
      const updateRes = await authenticatedApiClient.updateContent(SCHEMA, created.documentId, {
        name: 'Updated Cat Name',
        weight: 5.0,
      })
      expect(updateRes.ok()).toBeTruthy()

      const getRes = await authenticatedApiClient.getContent(SCHEMA, created.documentId)
    ...
    test('DELETE /content/:schema/:documentId deletes a document', async ({
      ...
      const deleteRes = await authenticatedApiClient.deleteContent(SCHEMA, created.documentId)
  ...
  test.describe('Publish / Unpublish', () => {
    test('publish and unpublish a document', async ({ authenticatedApiClient, cleanup }) => {
      const ownerData = testData.owner.create()
      const ownerRes = await authenticatedApiClient.createOwner(ownerData)
      const owner = await ownerRes.json()
      const ownerId = (owner.id ?? owner._id) as string
      ...
      // Publish
      const publishRes = await authenticatedApiClient.publishContent(SCHEMA, created.documentId)
      ...
      // Unpublish
      const unpublishRes = await authenticatedApiClient.unpublishContent(SCHEMA, created.documentId)
      expect(unpublishRes.ok()).toBeTruthy()
    })

    test('publish nonexistent document returns 404', async ({ authenticatedApiClient }) => {
      const response = await authenticatedApiClient.publishContent(SCHEMA, 'nonexistent-doc-id')
      expect(response.status()).toBe(404)
    })
  })
  ...
  test.describe('Authentication required', () => {
    test('GET /content/:schema requires authentication', async ({ request, apiBaseURL }) => {
      const response = await request.get(`${apiBaseURL}/content/${SCHEMA}`)
      expect(response.status()).toBe(401)
    })

    test('POST /content/:schema requires authentication', async ({ request, apiBaseURL }) => {

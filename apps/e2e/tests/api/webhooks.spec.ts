...
test.describe('Webhooks API', () => {
	test('CRUD flow — create, list, get, update, delete webhook', async ({
		...
		const found = webhooks.find(
			(w: { id?: string; _id?: string }) => (w.id || w._id) === webhookId,
		)
		...
		// Update
		const updateResponse = await authenticatedApiClient.updateWebhook(
			webhookId,
			{
	...
	test('POST /webhooks/:id/test sends test delivery', async ({
		...
		const deliveriesResponse =
			await authenticatedApiClient.getWebhookDeliveries(webhookId)
	...
	test('POST /webhooks/:id/regenerate-secret returns new secret', async ({
		...
		const regenResponse =
			await authenticatedApiClient.regenerateWebhookSecret(webhookId)
	...
	test('event-triggered delivery — content.created fires webhook', async ({
		...
		const contentResponse = await authenticatedApiClient.createContent(
			'owner',
			{
		...
		for (let i = 0; i < 10; i++) {
			...
			if (deliveriesResponse.ok()) {
				const deliveries = await deliveriesResponse.json()
				if (
					deliveries.items.some(
						(d: { event: string }) => d.event === 'content.created',
					)
				) {
					deliveryFound = true
					break

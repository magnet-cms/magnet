...
test.describe('Email Templates API', () => {
	test('GET /email-templates returns seeded templates', async ({
	...
	test('CRUD flow — create, get, update, delete', async ({
		authenticatedApiClient,
	}) => {
		...
		// Update
		const updateResponse = await authenticatedApiClient.updateEmailTemplate(
			id,
			{
	...
	test('GET /email-templates?category= filters by category', async ({
		authenticatedApiClient,
	}) => {
		const response = await authenticatedApiClient.listEmailTemplates({
			category: 'transactional',
	...
	test('GET /email-templates?search= filters by search term', async ({
		...
		const found = templates.some(
			(t: { slug: string; subject: string }) =>
				...
				t.subject.toLowerCase().includes('welcome'),
		)
	...
	test('POST /email-templates/:id/preview returns HTML and compiled subject', async ({
		...
		const previewResponse = await authenticatedApiClient.previewEmailTemplate(
			id,
			{
				name: 'Alice',
	...
	test('GET /email-templates/:id/versions returns version history', async ({
		...
		const versionsResponse =
			await authenticatedApiClient.getEmailTemplateVersions(id)
	...
	test('POST /email-templates/:id/test sends test email to admin', async ({
		...
		const testResponse = await authenticatedApiClient.testSendEmailTemplate(
			id,
			{
				name: 'Admin',
			},
		...
		expect(
			[200, 201, 204, 400, 500].includes(testResponse.status()),
		).toBeTruthy()
	...
	test('GET /email-templates/by-slug/:slug returns all locale variants', async ({
		...
		const bySlugResponse =
			await authenticatedApiClient.getEmailTemplatesBySlug(slug)

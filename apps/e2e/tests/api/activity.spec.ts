...
test.describe('Activity API', () => {
  test.describe('GET /activity', () => {
    test('returns recent activities array', async ({ authenticatedApiClient }) => {
  ...
  test.describe('GET /activity/entity/:type/:id', () => {
    test('returns activities for entity', async ({ authenticatedApiClient }) => {
      const response = await authenticatedApiClient.getActivityByEntity('content', 'some-doc-id')
      expect(response.ok()).toBeTruthy()
      const body = await response.json()
      expect(Array.isArray(body)).toBe(true)
    })
    ...
    test('requires authentication', async ({ request, apiBaseURL }) => {
      const response = await request.get(`${apiBaseURL}/activity/entity/content/some-id`)
      expect(response.status()).toBe(401)
    })
  })
  ...
  test.describe('GET /activity/search', () => {
    ...
    test('returns paginated result with items and total', async ({ authenticatedApiClient }) => {
      const response = await authenticatedApiClient.searchActivity({
        entityType: 'content',
        limit: 10,
        offset: 0,

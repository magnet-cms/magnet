...
test.describe('Environment API', () => {
  test('GET /environments returns all environments', async ({ authenticatedApiClient }) => {
  ...
  test('GET /environments includes local environment', async ({ authenticatedApiClient }) => {
    const response = await authenticatedApiClient.getEnvironments()
    const envs = await response.json()
  ...
  test('GET /environments/local returns local environment', async ({ authenticatedApiClient }) => {
  ...
  test.describe('Authentication required', () => {
    test('GET /environments requires authentication', async ({ request, apiBaseURL }) => {
      const response = await request.get(`${apiBaseURL}/environments`)
      expect(response.status()).toBe(401)
    })

    test('GET /environments/local requires authentication', async ({ request, apiBaseURL }) => {

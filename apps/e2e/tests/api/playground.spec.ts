...
test.describe('Playground API', () => {
  test.describe('Public access', () => {
    test('GET /playground/schemas is publicly accessible', async ({ request, apiBaseURL }) => {
  ...
  test.describe('CRUD operations', () => {
    test('GET /playground/schemas returns list', async ({ authenticatedApiClient }) => {
    ...
    test('POST /playground/schemas creates a schema', async ({
      ...
      const name = schemaName()
      const response = await authenticatedApiClient.createPlaygroundSchema(minimalSchema(name))
      ...
      cleanup.register(() => authenticatedApiClient.deletePlaygroundSchema(name))
    })
    ...
    test('PUT /playground/schemas/:name updates a schema', async ({
      ...
      const response = await authenticatedApiClient.updatePlaygroundSchema(name, updatedSchema)
      expect(response.ok()).toBeTruthy()
    ...
    test('DELETE /playground/schemas/:name removes the schema', async ({
      ...
      const deleteResponse = await authenticatedApiClient.deletePlaygroundSchema(name)
    ...
    test('created schema appears in list', async ({ authenticatedApiClient, cleanup }) => {
      const name = schemaName()
      await authenticatedApiClient.createPlaygroundSchema(minimalSchema(name))
      cleanup.register(() => authenticatedApiClient.deletePlaygroundSchema(name))
  ...
  test.describe('Error cases', () => {
    test('GET /playground/schemas/:name returns 404 for nonexistent schema', async ({
      ...
      const response = await authenticatedApiClient.getPlaygroundSchema('NonexistentSchema')
    ...
    test('POST /playground/schemas returns 409 for duplicate schema name', async ({
      ...
      const duplicateResponse = await authenticatedApiClient.createPlaygroundSchema(
        minimalSchema(name),
      )
    ...
    test('DELETE /playground/schemas/:name returns 404 for nonexistent schema', async ({
      ...
      const response = await authenticatedApiClient.deletePlaygroundSchema('NonexistentSchema')
      expect(response.status()).toBe(404)
    })
    ...
    test('PUT /playground/schemas/:name returns 404 for nonexistent schema', async ({
      ...
      const response = await authenticatedApiClient.updatePlaygroundSchema('NonexistentSchema', {
  ...
  test.describe('Preview endpoint', () => {
    test('POST /playground/preview generates code preview', async ({ authenticatedApiClient }) => {
      const name = schemaName('Preview')
      const response = await authenticatedApiClient.previewPlaygroundCode(minimalSchema(name))
    ...
    test('POST /playground/preview does not persist schema', async ({ authenticatedApiClient }) => {
      const name = schemaName('Preview')

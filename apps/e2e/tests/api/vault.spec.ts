...
test.describe('Vault API', () => {
  test('GET /vault/status returns healthy status', async ({ authenticatedApiClient }) => {
  ...
  test('Vault CRUD — create, read, update, delete secret', async ({ authenticatedApiClient }) => {
    ...
    test.skip(status.adapter === 'db', 'DB vault adapter uses different response shape')
    ...
    // Create secret
    const createResponse = await authenticatedApiClient.setVaultSecret(secretKey, secretData)
    ...
    // Update secret
    const updateResponse = await authenticatedApiClient.setVaultSecret(secretKey, updatedData)
    expect(updateResponse.ok()).toBeTruthy()

    // Verify update
    const verifyResponse = await authenticatedApiClient.getVaultSecret(secretKey)
    ...
    // Delete secret
    const deleteResponse = await authenticatedApiClient.deleteVaultSecret(secretKey)
    expect(deleteResponse.ok()).toBeTruthy()

    // Verify deletion — should return empty data
    const deletedResponse = await authenticatedApiClient.getVaultSecret(secretKey)

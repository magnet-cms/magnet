...
test.describe('Discovery API', () => {
  test.describe('Schema discovery', () => {
    test('GET /discovery/schemas responds within 5000ms', async ({ authenticatedApiClient }) => {
    ...
    test('GET /discovery/schemas returns list of schemas', async ({ authenticatedApiClient }) => {
      const response = await authenticatedApiClient.getSchemas()
      expect(response.ok()).toBeTruthy()
    ...
    test('GET /discovery/schemas/cat has expected fields', async ({ authenticatedApiClient }) => {
      const response = await authenticatedApiClient.getSchema('cat')
      const schema = await response.json()
  ...
  test.describe('Settings discovery', () => {
    test('GET /discovery/settings returns settings schemas', async ({ authenticatedApiClient }) => {
  ...
  test.describe('Public access', () => {
    test('GET /discovery/schemas is publicly accessible', async ({ request, apiBaseURL }) => {
      const response = await request.get(`${apiBaseURL}/discovery/schemas`)
      expect(response.ok()).toBeTruthy()
    })

    test('GET /discovery/controllers is publicly accessible', async ({ request, apiBaseURL }) => {

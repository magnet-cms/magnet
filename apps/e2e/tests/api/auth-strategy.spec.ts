...
test.describe('Auth Strategy System', () => {
  test.describe('Default JWT Strategy (Backward Compatibility)', () => {
    test('login returns access_token with JWT strategy', async ({ apiClient }) => {
    ...
    test('JWT token can be used to access protected endpoints', async ({ apiClient }) => {
      const status = await apiClient.getAuthStatus()
      test.skip(status.requiresSetup === true, 'Setup required first')
  ...
  test.describe('Auth Status Endpoint', () => {
    test('returns correct status for unauthenticated requests', async ({ apiClient }) => {
      const status = await apiClient.getAuthStatus()

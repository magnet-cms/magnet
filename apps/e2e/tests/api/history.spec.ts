...
test.describe('History API', () => {
  test.describe('Version CRUD', () => {
    test('POST /history/version creates a version entry', async ({
      ...
      cleanup.register(() => authenticatedApiClient.deleteVersion(getVersionId(version)))
    })
    ...
    test('GET /history/versions/:documentId returns versions for a document', async ({
      ...
      cleanup.register(() => authenticatedApiClient.deleteVersion(getVersionId(v1)))
      ...
      cleanup.register(() => authenticatedApiClient.deleteVersion(getVersionId(v2)))

      const response = await authenticatedApiClient.getVersions(docId, COLLECTION)
      expect(response.ok()).toBeTruthy()
    ...
    test('GET /history/versions/:documentId/latest returns latest version', async ({
      ...
      cleanup.register(() => authenticatedApiClient.deleteVersion(getVersionId(created)))

      const response = await authenticatedApiClient.getLatestVersion(docId, COLLECTION, 'draft')
      expect(response.ok()).toBeTruthy()
    ...
    test('DELETE /history/version/:id deletes a version', async ({ authenticatedApiClient }) => {
      const docId = makeDocId()
      const createRes = await authenticatedApiClient.createVersion({
        documentId: docId,
        collection: COLLECTION,
  ...
  test.describe('Authentication required', () => {
    test('GET /history/versions/:id requires authentication', async ({ request, apiBaseURL }) => {
    ...
    test('POST /history/version requires authentication', async ({ request, apiBaseURL }) => {

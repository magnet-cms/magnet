...
test.describe('Settings API', () => {
  test.describe('Read operations', () => {
    test('GET /settings returns all settings groups', async ({ authenticatedApiClient }) => {
    ...
    test('GET /settings/auth returns auth settings group', async ({ authenticatedApiClient }) => {
    ...
    test('GET /settings/auth/minPasswordLength returns a single setting', async ({
      ...
      const response = await authenticatedApiClient.getSetting('auth', 'minPasswordLength')
      expect(response.ok()).toBeTruthy()
    ...
    test('GET /settings/:group/:key returns 404 for nonexistent key', async ({
      ...
      const response = await authenticatedApiClient.getSetting('auth', 'nonExistentKey')
      expect(response.status()).toBe(404)
    })
  })

  test.describe('Update operations', () => {
    test('PUT /settings/auth updates auth settings', async ({ authenticatedApiClient }) => {
      // Read current value first
      const getRes = await authenticatedApiClient.getSetting('auth', 'maxLoginAttempts')
      ...
      const verifyRes = await authenticatedApiClient.getSetting('auth', 'maxLoginAttempts')
    ...
    test('PUT /settings/:group/:key updates a single setting', async ({
      ...
      const getRes = await authenticatedApiClient.getSetting('auth', 'lockoutDuration')
      const current = await getRes.json()
      const originalValue = current.value as number
      ...
      const verifyRes = await authenticatedApiClient.getSetting('auth', 'lockoutDuration')
      const verified = await verifyRes.json()
      expect(verified.value).toBe(newValue)

      // Restore original value
      await authenticatedApiClient.updateSetting('auth', 'lockoutDuration', originalValue)
    })
  })
  ...
  test.describe('Default values', () => {
    test('auth settings have correct default values after initialization', async ({
      ...
      // Verify auth settings keys exist with correct types
      // (exact values may differ due to parallel test updates)
      expect(typeof byKey.sessionDuration).toBe('number')
      expect(typeof byKey.refreshTokenDuration).toBe('number')
      expect(typeof byKey.enableSessions).toBe('boolean')
      expect(typeof byKey.minPasswordLength).toBe('number')
      expect(typeof byKey.requireUppercase).toBe('boolean')
      expect(typeof byKey.requireNumber).toBe('boolean')
      expect(typeof byKey.requireSpecialChar).toBe('boolean')
      expect(typeof byKey.maxLoginAttempts).toBe('number')
      expect(typeof byKey.lockoutDuration).toBe('number')
      expect(typeof byKey.allowRegistration).toBe('boolean')
      expect(typeof byKey.requireEmailVerification).toBe('boolean')
    })
  })
})

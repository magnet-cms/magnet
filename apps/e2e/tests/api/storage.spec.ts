...
function createTestPng(): Buffer {
  ...
  const ihdr = Buffer.from([
    ...
    0x08, 0x02, 0x00, 0x00, 0x00, 0x90, 0x77, 0x53, 0xde, 0x00,
  ])
...
test.describe('Storage API', () => {
  ...
  test('upload file and verify metadata', async ({ authenticatedApiClient, cleanup }) => {
    const filename = `e2e-test-${Date.now()}.png`

    const response = await authenticatedApiClient.uploadMedia(testPng, filename, 'image/png', {
      alt: 'E2E test image',
      tags: ['e2e', 'test'],
    })
  ...
  test('upload and list files', async ({ authenticatedApiClient, cleanup }) => {
    ...
    // Upload
    const uploadResponse = await authenticatedApiClient.uploadMedia(testPng, filename, 'image/png')
    ...
    const found = list.items.find(
      (item: { originalFilename: string }) => item.originalFilename === filename,
    )
    expect(found).toBeTruthy()
  })

  test('upload and retrieve file content', async ({ authenticatedApiClient, cleanup }) => {
  ...
  test('upload with folder organizes files', async ({ authenticatedApiClient, cleanup }) => {
  ...
  test('template-specific URL format', async ({ authenticatedApiClient, cleanup }) => {

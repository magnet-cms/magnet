...
test.describe('RBAC API', () => {
  test.describe('Roles', () => {
    ...
    test('PUT /rbac/roles/:id updates a role', async ({ apiClient }) => {
      ...
      const updateResponse = await apiClient.updateRole(createdRole.id, updateData)
    ...
    test('DELETE /rbac/roles/:id cannot delete system roles', async ({ apiClient }) => {
      const status = await apiClient.getAuthStatus()
      test.skip(status.requiresSetup === true, 'Setup required first')
    ...
    test('POST /rbac/roles/:id/duplicate duplicates a role', async ({ apiClient }) => {
      const status = await apiClient.getAuthStatus()
      test.skip(status.requiresSetup === true, 'Setup required first')
      ...
      const duplicateResponse = await apiClient.duplicateRole(sourceRole.id, duplicateData)
    ...
    test('PUT /rbac/roles/:id/permissions updates role permissions', async ({
      ...
      if (permData && typeof permData === 'object' && !Array.isArray(permData)) {
          ...
          if (Array.isArray(category)) {
            for (const entry of category) {
              if (entry.permissions) {
                allPerms.push(...entry.permissions.map((p: { id: string }) => p.id))
              }
            }
          }
        }
      ...
      const basePerms = allPerms.slice(0, 2).length > 0 ? allPerms.slice(0, 2) : ['*']
      const newPermissions = allPerms.slice(0, 3).length > 0 ? allPerms.slice(0, 3) : ['*']
  ...
  test.describe('Permissions', () => {
    test('GET /rbac/permissions returns categorized permissions', async ({ apiClient }) => {
      const status = await apiClient.getAuthStatus()
      test.skip(status.requiresSetup === true, 'Setup required first')
    ...
    test('GET /rbac/my-permissions returns current user permissions', async ({ apiClient }) => {
      const status = await apiClient.getAuthStatus()
      test.skip(status.requiresSetup === true, 'Setup required first')
    ...
    test('GET /rbac/check/:permission checks a specific permission', async ({ apiClient }) => {
      const status = await apiClient.getAuthStatus()
      test.skip(status.requiresSetup === true, 'Setup required first')
  ...
  test.describe('User Role Assignment', () => {
    test('PUT /rbac/users/:userId/role assigns role to user', async ({ apiClient }) => {
      const status = await apiClient.getAuthStatus()
      test.skip(status.requiresSetup === true, 'Setup required first')
      ...
      const assignResponse = await apiClient.assignUserRole(adminUser.id, role.name)
  ...
  test.describe('Authentication Required', () => {
    test('GET /rbac/roles requires authentication', async ({ request, apiBaseURL }) => {
      const response = await request.get(`${apiBaseURL}/rbac/roles`)
      expect(response.status()).toBe(401)
    })

    test('GET /rbac/permissions requires authentication', async ({ request, apiBaseURL }) => {
      const response = await request.get(`${apiBaseURL}/rbac/permissions`)
      expect(response.status()).toBe(401)
    })

    test('GET /rbac/my-permissions requires authentication', async ({ request, apiBaseURL }) => {

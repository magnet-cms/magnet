...
authTest.describe('Admin Error Boundary', () => {
	...
	authTest(
		'should load email templates listing page without crashing',
		async ({ page }) => {
			...
			const insertNodesErrors = pageErrors.filter((e) =>
	...
	authTest(
		'should load email template editor (new) without crashing',
		async ({ page }) => {
			const pageErrors: string[] = []
			page.on('pageerror', (err) => {
				pageErrors.push(err.message)
			...
			const referenceErrors = pageErrors.filter(
				(e) =>
					e.includes('$insertNodes is not defined') ||

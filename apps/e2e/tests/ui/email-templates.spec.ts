...
authTest.describe('Email Templates UI', () => {
  ...
  authTest('email templates list page loads', async ({ page }) => {
    await page.goto(`${process.env.UI_BASE_URL || 'http://localhost:3001'}/email-templates`)
  ...
  authTest('can filter templates by category', async ({ page }) => {
    ...
    const categoryTrigger = page.getByRole('combobox').or(page.locator('[role="combobox"]')).first()
  ...
  authTest('editor page loads for new template', async ({ page }) => {
    await page.goto(`${process.env.UI_BASE_URL || 'http://localhost:3001'}/email-templates/new`)
    await page.waitForLoadState('networkidle')

    // Slug uses placeholder "welcome"; subject uses a Handlebars example string
    await expect(page.locator('input[placeholder="welcome"]').first()).toBeVisible({
      timeout: 10000,
    })
    await expect(page.locator('input[placeholder^="Welcome{{"]').first()).toBeVisible()
  })

  authTest('edit action navigates to editor page', async ({ page, authenticatedApiClient }) => {
    ...
    const actionsMenu = page.locator('tr', { hasText: slug }).getByRole('button').last()

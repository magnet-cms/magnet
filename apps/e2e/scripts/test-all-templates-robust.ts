...
import { examples, startTemplate, stopAllTemplates, stopTemplate } from './docker-manager'
...
function updateDevAdmin(example: ExampleName, envVars: Record<string, string>): void {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 500) {
        ...
        if (text.includes('statusCode') || text.includes('message')) {
          ...
          console.log('⚠️  Health endpoint returned 500, but server is responding')
          return true
        }
      }
    } catch {
...
async function runTestsForExample(example: ExampleName, testArgs: string[] = []): Promise<boolean> {
  console.log(`\n${'='.repeat(70)}`)
  ...
  try {
    ...
    console.log('⏳ Waiting for backend server to be ready (up to 180 seconds)...')
...
async function main() {
  ...
  const example = exampleArg ? (exampleArg.split('=')[1] as ExampleName) : undefined
  ...
  if (serversAlreadyRunning) {
    console.log('🧪 Running e2e tests (servers assumed already running on 3000/3001)...\n')
  ...
  if (!dockerOk) {
    ...
    console.error('')
    console.error('   To run tests against already-running servers instead, use:')
  ...
  if (example) {
    ...
    const results: Record<ExampleName, boolean> = {} as Record<ExampleName, boolean>
    let attempt = 1
    const maxAttempts = 3

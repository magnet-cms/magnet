...
const examples: Record<string, ExampleConfig> = {
  mongoose: {
    composeFile: resolve(__dirname, '../../examples/mongoose/docker/docker-compose.yml'),
    port: 27017,
    service: 'mongodb',
    additionalChecks: [
      {
  ...
  'drizzle-neon': {
    composeFile: resolve(__dirname, '../../examples/drizzle-neon/docker/docker-compose.yml'),
    port: 5433,
    service: 'postgres',
    additionalChecks: [
      {
  ...
  'drizzle-supabase': {
    composeFile: resolve(__dirname, '../../examples/drizzle-supabase/docker/docker-compose.yml'),
    port: 5432,
...
async function waitForService(check: ServiceHealthCheck, maxAttempts: number): Promise<void> {
  let attempts = 0
  ...
  throw new Error(`${check.container} did not become ready within ${maxAttempts} seconds`)
}
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      console.error('Usage: bun docker-manager.ts <start|stop|stop-all> [example]')
      process.exit(1)
  }
}

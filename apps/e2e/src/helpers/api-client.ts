...
export class ApiClient {
  ...
  async updateMedia(id: string, data: { alt?: string; tags?: string[]; folder?: string }) {
  ...
  async deleteMediaFolder(path: string) {
    return this.request.delete(`${this.baseURL}/media/folders/${encodeURIComponent(path)}`, {
      headers: this.getHeaders(),
    })
  }
  ...
  async getMediaFile(id: string, transform?: { w?: number; h?: number; f?: string; q?: number }) {
    const params = new URLSearchParams()
  ...
  async listContent(schema: string, options?: { locale?: string; status?: string }) {
    const params = new URLSearchParams()
  ...
  async getContent(schema: string, documentId: string, options?: { locale?: string }) {
    const params = new URLSearchParams()
    if (options?.locale) params.set('locale', options.locale)
  ...
  async createEmptyContent(schema: string, options?: { locale?: string; createdBy?: string }) {
  ...
  async unpublishContent(schema: string, documentId: string, options?: { locale?: string }) {
    const params = new URLSearchParams()
    if (options?.locale) params.set('locale', options.locale)
  ...
  async addContentLocale(
    ...
    return this.request.post(`${this.baseURL}/content/${schema}/${documentId}/locale`, {
      headers: this.getHeaders(),
      data: { locale, data, createdBy: options?.createdBy },
    })
  }

  async deleteContentLocale(schema: string, documentId: string, locale: string) {
    return this.request.delete(`${this.baseURL}/content/${schema}/${documentId}/locale/${locale}`, {
      headers: this.getHeaders(),
    })
  }

  async getContentLocaleStatuses(schema: string, documentId: string) {
    return this.request.get(`${this.baseURL}/content/${schema}/${documentId}/locales`, {
      headers: this.getHeaders(),
    })
  }

  async getContentVersions(schema: string, documentId: string, options?: { locale?: string }) {
    const params = new URLSearchParams()
    if (options?.locale) params.set('locale', options.locale)
  ...
  async restoreContentVersion(schema: string, documentId: string, locale: string, version: number) {
  ...
  async deleteContent(schema: string, documentId: string) {
    return this.request.delete(`${this.baseURL}/content/${schema}/${documentId}`, {
      headers: this.getHeaders(),
    })
  }
  ...
  async rotateApiKey(id: string): Promise<CreatedApiKeyResponse> {
    const response = await this.request.post(`${this.baseURL}/api/api-keys/${id}/rotate`, {
      headers: this.getHeaders(),
    })
    return response.json()
  }
  ...
  async publishVersion(versionId: string) {
    return this.request.put(`${this.baseURL}/history/version/${versionId}/publish`, {
      headers: this.getHeaders(),
    })
  }

  async archiveVersion(versionId: string) {
    return this.request.put(`${this.baseURL}/history/version/${versionId}/archive`, {
      headers: this.getHeaders(),
    })
  }
  ...
  async getActivityByEntity(entityType: string, entityId: string) {
    return this.request.get(`${this.baseURL}/activity/entity/${entityType}/${entityId}`, {
      headers: this.getHeaders(),
    })
  }
  ...
  async searchActivity(params: {
    ...
    return this.request.get(`${this.baseURL}/activity/search${qs ? `?${qs}` : ''}`, {
      headers: this.getHeaders(),
    })
  }

  async compareVersions(versionId1: string, versionId2: string) {
    return this.request.get(`${this.baseURL}/history/compare/${versionId1}/${versionId2}`, {
      headers: this.getHeaders(),
    })
  }
  ...
  async getVaultSecret(key: string) {
    return this.request.get(`${this.baseURL}/vault/secrets/${encodeURIComponent(key)}`, {
      headers: this.getHeaders(),
    })
  }

  async setVaultSecret(key: string, data: Record<string, unknown>) {
    return this.request.post(`${this.baseURL}/vault/secrets/${encodeURIComponent(key)}`, {
      headers: this.getHeaders(),
      data: { data },
    })
  }
  ...
  async deleteVaultSecret(key: string) {
    return this.request.delete(`${this.baseURL}/vault/secrets/${encodeURIComponent(key)}`, {
      headers: this.getHeaders(),
    })
  }
  ...
  async regenerateWebhookSecret(id: string) {
    return this.request.post(`${this.baseURL}/webhooks/${id}/regenerate-secret`, {
      headers: this.getHeaders(),
    })
  }
  ...
  async getViewConfig(schema: string) {
    return this.request.get(`${this.baseURL}/user-preferences/view-config/${schema}`, {
      headers: this.getHeaders(),
    })
  }
  ...
  async putViewConfig(
    ...
    return this.request.put(`${this.baseURL}/user-preferences/view-config/${schema}`, {
      headers: this.getHeaders(),
      data,
    })
  }
  ...
  async listEmailTemplates(params?: { category?: string; search?: string; locale?: string }) {
    const qs = new URLSearchParams()
    if (params?.category) qs.set('category', params.category)
    if (params?.search) qs.set('search', params.search)
    if (params?.locale) qs.set('locale', params.locale)
    const query = qs.toString()
    return this.request.get(`${this.baseURL}/email-templates${query ? `?${query}` : ''}`, {
      headers: this.getHeaders(),
    })
  }
  ...
  async cleanupNotifications(retentionDays?: number) {
    const qs = retentionDays !== undefined ? `?retentionDays=${retentionDays}` : ''
